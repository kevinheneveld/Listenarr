...
namespace Listenarr.Application.Audiobooks.Verification
{
    ...
    /// with plenty of speech and no trace of either the title or the author is
    /// obvious junk. Match-confirmation is the lower-confidence path, and
    /// everything ambiguous (music intro, self-narrated indie with no credits,
    /// foreign language) lands in Uncertain for human review rather than being
    /// guessed at.
    /// </summary>
    public class DeterministicIdentityVerifier : IIdentityVerifier
    {
        ...
        internal VerificationVerdict Evaluate(Audiobook audiobook, string? openingText, string? closingText)
        {
            ...
            if (tokens.Count < TranscriptMatcher.MinUsableTranscriptTokens)
            {
                ...
                // silence, no spoken credits). Never call this a mismatch.
                outcome = VerificationOutcome.Uncertain;
                confidence = 0.15;
            }
            ...
            {
                // Plenty of speech, no trace of title OR author: the obvious junk
                // this feature exists to catch. This is the high-confidence signal.
