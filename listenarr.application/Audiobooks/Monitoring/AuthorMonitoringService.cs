...
namespace Listenarr.Application.Audiobooks.Monitoring
{
    public partial class AuthorMonitoringService : IAuthorMonitoringService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                // A monitored author is unique per (name, region, language). Changing the
                // language could collide with an existing row for the same author (e.g.
                // correcting a stale "all" to "english" when an "english" row was also
                // created): drop that duplicate and repurpose this record so we end up with
                // a single row, rather than failing on the unique index.
            ...
            // Re-sync so the change takes effect immediately: a narrowed language stops
            // adding off-language books on the next sweep; this pass backfills any now-in-
            // language books that are missing (like the initial monitor). Books already
            // added under the old language are not removed — matching the sweep's add-only
            // contract.
