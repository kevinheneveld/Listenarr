        ...
        public async Task<object?> GetMetadataAsync(string asin, string region = "us", bool cache = true)
        {
            ...
            foreach (var source in metadataSources)
            {
                try
                {
                    ...
                    // non-null but EMPTY product stub (no title, no anything)
                    // — treating that as an answer both returns junk to the
                    // caller and blocks every later source in the chain
                    // (live case: an Amazon-only edition never reached the
                    // Amazon fallback because the empty Audible shell "won").
