...
namespace Listenarr.Application.Common.Images
{
    ...
    /// The add-time hooks and the historical-backlog sweep service call through
    /// here so the cache key derivation and the external-URL predicate stay in
    /// one place.
    /// </summary>
    public static class LibraryImageStorageHelper
    {
        ...
        /// title + first author. ASIN-first mirrors the keying used by the add-time
        /// hooks so the sweep doesn't shard the same record across two different files.
        /// </summary>
