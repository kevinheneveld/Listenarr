...
namespace Listenarr.Application.Common.Images
{
    ...
    public sealed class ExternalCoverArtSweepService : IExternalCoverArtSweepService
    {
        ...
        // pattern. 250ms ≈ 4 downloads/sec, plenty for a few-thousand-book
        // library and well under what an unauthenticated CDN endpoint should
        // care about.
        ...
        public async Task<ExternalCoverArtSweepResult> SweepAsync(CancellationToken cancellationToken = default)
        {
            ...
            foreach (var audiobook in audiobooks)
            {
                ...
                {
                    // Leave the external URL in place — matches the add-time
                    // hooks' fallback behavior and avoids a needless write.
                    failed++;
                    _logger.LogWarning(
                ...
                try
                {
                    // Targeted write: the sweep iterates a snapshot taken at start;
                    // re-saving the whole entity would clobber concurrent edits made
                    // while the (potentially long) sweep runs.
                    await _repository.SetImageUrlAsync(audiobook.Id, movedPath);
                    succeeded++;
                }
                catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                {
        ...
        /// "https://.../config/cache/...".
        /// </summary>
        internal static bool ShouldCacheLocally(string? url)
        {
