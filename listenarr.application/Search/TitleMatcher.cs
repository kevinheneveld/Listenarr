...
{
    /// <summary>
    /// Punctuation-tolerant title matching used when narrowing Audible
    /// author-search results down to a specific title. A naive
    /// case-insensitive substring check fails on common punctuation
    /// mismatches between user-entered titles and Audible's canonical
    /// titles (hyphen vs. colon, en/em dashes, smart quotes, doubled
    /// whitespace), causing legitimate matches to be silently dropped.
    /// </summary>
    public static class TitleMatcher
    {
        ...
        public static bool Matches(string? title, string? subtitle, string? query)
        {
            ...
            var candidateTokens = new HashSet<string>(
                SignificantTokens.From(combined),
                StringComparer.OrdinalIgnoreCase);
