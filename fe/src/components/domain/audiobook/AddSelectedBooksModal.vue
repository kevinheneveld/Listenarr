    ...
    <div v-if="visible" class="modal-overlay" @click.self="close">
      <div class="modal modal-md" role="dialog" aria-modal="true">
        ...
        <div class="modal-body">
          <p class="intro">
            {{ books.length }} missing book{{ books.length === 1 ? '' : 's' }} in this series. Uncheck
            anything you don't want (duplicates, editions you don't care about), then add.
          </p>
          ...
          <div class="options-section">
            <div class="option-group">
              ...
              <RootFolderSelect v-model:rootId="selectedRootId" v-model:customPath="customRootPath" hideLabel />
            </div>
            <div class="option-group">
          ...
          <div v-if="adding || progress.added || progress.skipped || progress.failed" class="progress-row">
            <PhSpinner v-if="adding" class="spin" />
            <PhCheckCircle v-else class="success" />
            <span>
              {{ progress.added }} added<template v-if="progress.skipped">, {{ progress.skipped }} already had</template><template v-if="progress.failed">, {{ progress.failed }} failed</template>
              of {{ progress.total }}
            </span>
          </div>
        </div>
