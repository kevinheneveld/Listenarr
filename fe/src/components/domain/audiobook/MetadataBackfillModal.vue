...
<!--
  ...
  Follow-ups (intentionally NOT in this PR — coming as separate changes):
    - Reading the file's embedded composer tag to rank candidates whose
      narrator matches the file's narrator. Improves the multi-narrator
      disambiguation case but requires a new endpoint that reads ffprobe
      tags, so it lands separately.
    - Auditioning the owned audio file inline so the user can identify
      the narrator before picking a candidate. Requires a FilePreviewModal
      component and the same embedded-tag endpoint above.
-->
...
// Non-immediate on purpose: the modal is intended to be mounted with
// `visible: false` and toggled to true on user action. Callers that mount
// with `visible: true` from the initial render won't trigger `start()`.
watch(
  () => props.visible,
  (visible) => {
    if (!visible) return
...
//
// Uses the audiobook's current `narrators` DB field (set by a prior backfill,
// manual edit, or upstream metadata import) to rank candidates whose narrators
// match. Matching is case- and punctuation-insensitive with substring in
// either direction so "Pike" matches "Rosamund Pike".
