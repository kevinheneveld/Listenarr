...
async function loadCandidates(query?: string) {
  ...
  try {
    ...
    selectedAsin.value = result.bestGuessAsin ?? result.candidates[0]?.asin ?? null
  } catch (err) {
...
async function applyAsin(asin: string) {
  ...
  try {
    const catalog = await apiService.selectSeries(props.seriesName, chosen, props.region || 'us')
    if (!catalog || !catalog.series?.asin) {
      toast.error(
        'Could not load that series',
        'Audible returned no catalog for the selected series.',
      )
      return
    }
      ...
      <div class="modal modal-md" role="dialog" aria-modal="true">
        ...
        <div class="modal-body">
          ...
          <div class="picker-search">
            ...
            />
            <button class="btn" :disabled="loading || submitting" @click="runSearch">Search</button>
          </div>
        ...
        <footer class="modal-footer">
          ...
          <button class="btn btn-primary" :disabled="!selectedAsin || submitting" @click="confirm">
            <PhSpinner v-if="submitting" class="spin" :size="16" />
