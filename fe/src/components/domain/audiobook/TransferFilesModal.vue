...
<!--
  ...
  remediation when verification shows the files belong to a different book the
  library already tracks (often still "wanted"). Complements ExtractFileModal,
  which creates a NEW record from the catalog for a file.
-->
<template>
      ...
      <template v-else>
        <div class="transfer-section">
          ...
          <label v-for="f in sourceFiles" :key="f.id" class="transfer-file-row">
            <input type="checkbox" :checked="selectedFileIds.has(f.id)" @change="toggleFile(f.id)" />
        ...
        <div class="transfer-note">
          Files keep their audio untouched: ownership moves to the destination record (and the
          file relocates into its folder when possible). The destination is re-verified
