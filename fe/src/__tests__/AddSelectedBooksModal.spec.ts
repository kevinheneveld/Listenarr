...
vi.mock('@/stores/configuration', () => ({
  useConfigurationStore: () => ({ qualityProfiles: [{ id: 1, name: 'Default', isDefault: true }], loadQualityProfiles: vi.fn() }),
}))
vi.mock('@/stores/rootFolders', () => ({
  useRootFoldersStore: () => ({ folders: [{ id: 1, path: '/audiobooks', isDefault: true }], load: vi.fn() }),
}))
...
describe('AddSelectedBooksModal', () => {
  ...
  it('adds selected books and treats a 409 as a benign skip (not a failure)', async () => {
    ...
    const done = wrapper.emitted('done')?.[0]?.[0] as { added: number; skipped: number; failed: number }
