  ...
  const apiService = {
    ...
    getVerificationQueueStatus: vi.fn(async () => ({
      verification: {
        queuedJobs: 0,
        queuedBooks: 0,
        hasUnknownSizedJobs: false,
        processing: null,
        completedBooksToday: 0,
        avgSecondsPerBook: 180,
        etaSeconds: null,
      },
      seriesBackfill: { cached: 0, totalMultiBook: 0 },
    })),
    runSeriesCatalogBackfill: vi.fn(async () => ({
      message: '',
      totalSeries: 0,
      alreadyCached: 0,
      fetched: 0,
    ...
    previewRenameAudiobook: vi.fn(async () => ({
      audiobookId: 0,
      folderChanged: false,
      fileRenames: [],
      hasChanges: false,
    })),
    executeRenameAudiobook: vi.fn(async () => ({
      audiobookId: 0,
      success: true,
      renamedFiles: [],
    })),
    ...
    scanAudiobook: vi.fn(async () => ({ message: '', found: 0, created: 0 })),
