...
<template>
  <div class="library-search-view">
    ...
    <div
      v-else-if="libraryStore.loading && libraryStore.audiobooks.length === 0"
      class="search-state"
    >
...
const tabs = computed(() => {
  ...
  if (results.value.books.length)
    out.push({ key: 'book', label: 'Books', count: results.value.books.length })
  if (results.value.series.length)
    out.push({ key: 'series', label: 'Series', count: results.value.series.length })
  if (results.value.authors.length)
    out.push({ key: 'author', label: 'Authors', count: results.value.authors.length })
...
function sortFacets(list: LibraryFacetMatch[], byDefault: boolean): LibraryFacetMatch[] {
  ...
  return [...list].sort((a, b) => {
    if (key === 'count') {
      ...
      return (
        (b.count - a.count) * (byDefault ? 1 : dir === 1 ? 1 : -1) || a.name.localeCompare(b.name)
      )
    }
