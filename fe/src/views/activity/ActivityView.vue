    ...
    <div class="activity-summary">
      ...
      >
        ...
        <span class="chip-label"
          >{{ chip.label }}<span v-if="chip.windowed" class="chip-window"> · {{ windowHours }}h</span></span
        >
      </button>
    </div>
...
const summaryChips = computed<SummaryChip[]>(() => {
  ...
  return [
    { key: 'inprogress', label: 'In progress', category: 'InProgress', count: s?.inProgress ?? 0, windowed: false },
    { key: 'blocked', label: 'Blocked', category: 'Blocked', count: s?.blocked ?? 0, windowed: false },
    { key: 'imported', label: 'Imported', category: 'Imported', count: s?.imported ?? 0, windowed: true },
    { key: 'failed', label: 'Failed', category: 'Failed', count: s?.failed ?? 0, windowed: true },
    { key: 'stalled', label: 'Stalled', category: 'Stalled', count: s?.stalled ?? 0, windowed: true },
  ]
})
