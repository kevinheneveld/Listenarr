...
- **Cache external cover art locally on save (opt-in):** `PUT /library/{id}` accepts a new `cacheImageLocally` query parameter. When `true` and the request changes `ImageUrl` to an external `http(s)://` URL, the controller downloads the cover into local library storage and rewrites the stored URL to the local cache path (mirroring the existing add-path's `MoveMetadataImageToLibraryStorageAsync` hook). No-op when the URL is unchanged or already local. The metadata-backfill modal always sends the flag — the whole point of that flow is to import fresh metadata into the library. The edit-audiobook modal exposes a new "Download and cache cover art locally on save" checkbox under the Cover Image URL field, shown only when the URL is external and defaulted ON so the common case (paste-and-save) just works. Cache failures fall back to storing the external URL, matching the add-path. The historical-backlog sweep (covering existing rows that already have external URLs) is deliberately left as a separate concern.
