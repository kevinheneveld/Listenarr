...
### Added
- **Language filtering for search results:** The scorer now rejects editions whose language is not among the quality profile's preferred languages. Detection is conservative — it acts only on explicit markers (native-language words like `deutsch`/`español`, foreign words for "audiobook" like `hörbuch`/`audiolibro`, bracketed/dashed country codes like `[PL]`/`-DK-`, and language names in bracketed or "X Edition" constructions), and an explicit English marker anywhere (including foreign words for "English", e.g. a release tagged `[Английский]`) resolves to English. Language names appearing bare in legitimate English titles ("The Russian", "French Kiss", "Ancient Greek Literature") are deliberately not flagged. The filter works for any configured preferred language, not just English — a Spanish-only profile keeps Spanish releases and rejects English ones.
- **Pre-ingest verification:** A last-line check runs on a completed download after its files are on disk but before they are committed to the library, catching what the search-time scorer can miss because it only sees release titles. It inspects ffprobe metadata for every audio file and rejects the download when (a) the batch looks like a music album — at least 10 audio files with a median track length under 5 minutes — or (b) an audio file carries an explicit language tag that isn't among the profile's preferred languages. Conservative by design: it acts only on hard ffprobe signal and fails open whenever that signal is absent (no durations, no language tag). ffprobe metadata extraction now also reads the `language` tag.
