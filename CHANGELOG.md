...
### Added
- **Per-file rename on the audiobook detail page:** Pencil icon on each row of the Files tab opens a modal for renaming a single tracked file. The filename input preserves the original extension and rejects path separators; the existing `POST /api/v1/library/{id}/rename` endpoint accepts the single-file operation and moves the file on disk while updating the library record.
