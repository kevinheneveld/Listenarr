...
### Added
- **Find duplicate audiobooks (Library Maintenance):** New `GET /library/duplicates` returns groups of audiobook rows sharing a normalized (UPPER, trimmed) ASIN, with a server-suggested winner per group (prefers rows with files, then most files, then a real book folder, then the lowest Id). Each row carries authors, narrators, runtime, tracked-file list (path / size / format / bitrate / duration) and total size so users can verify two rows really are the same book before merging — same ASIN doesn't always mean same content. Each group includes a human-readable recommendation reason ("only row with files", "most tracked files (12 vs 1)", etc.). New `POST /library/duplicates/merge` consolidates groups in a single transaction with per-row actions: optional `WinnerId` (nullable), `LoserIds` (deleted, with `Downloads.AudiobookId` / `History.AudiobookId` / `MoveJobs.AudiobookId` reassigned to the winner), and `ClearAsinIds` (kept as rows, ASIN nulled so they stop appearing as duplicates) — for the "wrong ASIN got stamped on this row" case. Validation requires all losers and clear-asin ids in a pair to share the reference ASIN; on first mismatch the whole request is rejected without changes. Files on disk are not touched. Surfaced in Settings → General → Library Maintenance via a "Review duplicates…" button that opens a modal with per-row actions (Skip / Keep / Merge / Clear ASIN — all default to Skip), accordion-expandable file details, and an "open in new tab" link to each row's audiobook detail page. See kevinheneveld/Listenarr#6.
