...
- **Dashboard drill-down into the audiobooks library:** Every "missing X" count on the dashboard (the per-field bars in metadata completeness and the "Books missing" overview tile) is now clickable and deep-links into the audiobooks page with a preset filter applied (`/audiobooks?missing=coverArt`, etc.), shown as a clearable chip at the top of the list. The audiobooks page already loads the library client-side, so the drill-down adds no new backend endpoint. Stacked have/missing bars were also added to the genres and duration distribution sections (matching the authors/narrators pattern), and the activity time-series now shows two overlaid lines — *Requested* (books added to the library) and *Imported* (files actually arrived) — so the queue-to-arrival lag is visible.
