...
### Added
- **Library metrics dashboard:** A new `/dashboard` page (and nav entry) presents aggregate metrics for the whole library, served by a new `GET /api/v1/dashboard/stats` endpoint. Sections cover overview counts (books, files, size, total/average duration), per-field metadata completeness with an overall score, series completeness (owned vs. the cached Audible catalog, with uncached series shown as "unknown" rather than guessed), authors & narrators, file quality (codec and bitrate), genre/duration/language distributions, and a books-added activity time-series with an adjustable Day/Week/Month granularity. Charts use ApexCharts; the route and chart library are lazy-loaded so they add nothing to the initial bundle.
