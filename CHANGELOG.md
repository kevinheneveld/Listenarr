...
### Added
- **Audiobook delete dialog: a "Not an audiobook — find a better match" action.** When the wrong content gets imported for a book (e.g. a music single that matched the title), deleting the whole audiobook record is the wrong tool — it removes the entry entirely, so nothing is left to re-search. The Delete dialog now offers a distinct, non-destructive action (`POST /library/{id}/not-audiobook`) that removes every tracked file from disk + the library, keeps the audiobook record and re-monitors it, then kicks off a fresh search. Because the improved `AudiobookOnlyFilter` (below) runs in every search path, the same non-audiobook release is not re-grabbed, so this finds a correct release or correctly leaves the book wanted rather than re-importing the junk. The action lives in a shared `NotAudiobookAction` component so it appears in the delete dialog from **all** library views — the book detail page, the Books list/grid, and collection/series pages. Unit-covered (removes every file with the `not-audiobook` source, re-monitors, triggers search; degrades gracefully to file-removal-only when the search invoker is unavailable).
- **Organize library: the "Invalid target" list is now grouped, explained, and links to each record.** The skipped-rows section of the Organize library modal previously printed a flat list that repeated the same long sentence on every row and offered no way to act on it. Rows are now grouped by problem (a machine-readable `reasonCode` accompanies the human `reason` on each `invalid_target` preview row — e.g. `target_exists`, `target_ancestor`, `missing_author`, `source_at_root`), each group showing the count, a one-line explanation, and concrete how-to-resolve guidance (route true duplicates to the Duplicates tool, flatten a nested-too-deep folder, re-scan a root-stamped BasePath, fix missing metadata). Each row links to its audiobook detail page ("id N ↗", opening in a new tab so the preview survives) and now shows both the **from** (current) and **to** (canonical target) paths — `targetPath` is surfaced on the ancestor/target-exists invalid rows so you can see exactly which folder is in the way. The presentation change is additive (`reasonCode`/`targetPath` on the preview DTO) and doesn't alter what counts as invalid. **The "nested one level too deep" group also gains a one-click "Flatten into canonical folder" action** (`POST /library/organize/flatten`): these rows are a historical import artifact where a book's files sit in a redundant subfolder beneath their canonical folder, which the normal move queue can't fix because the target is an ancestor of the source (`MoveExecutor` refuses that). The new `MoveExecutor.ExecuteFlatten` collapses the wrapper directories via two same-volume renames through a sibling staging dir, strongly guarded so no real file is ever clobbered or lost — it refuses unless the target is a strict ancestor of the source *and* every file under the target already lives under the source (i.e. the target holds nothing but the source's subtree wrapped in empty directories). On success the audiobook's BasePath and file paths are rebased in place (mirroring the organize/rename path-rebase) so no re-scan is needed, and a History "Organized" entry is written. The preview disk-checks each nested row up front (shared `MoveExecutor.EvaluateFlatten`, used by both the preview and the executor so they can't disagree): the "Flatten" button only appears when the collapse would actually succeed (`canFlatten`), nested rows whose canonical folder already holds other files show why the button is absent instead, and a record whose folder has vanished from disk is split out into its own **"Files missing on disk"** (`source_missing`) group pointing you to a re-scan rather than masquerading as flatten-able.
- **Narrators are now a first-class way to browse your library.** The library's group-by control gains a **Narrators** option alongside Books / Authors / Series, listing every narrator you own as a card (book-cover mosaic, like series) that opens a per-narrator collection page. Because a book can credit several narrators, each book fans out to every person who narrated it (matched case- and punctuation-insensitively, so casing variants merge), while non-person production credits like "Full Cast" are excluded so the list stays browsable people. The per-narrator collection page is library-only (there is no narrator catalog to "add missing" from) and reuses the existing collection view. No backend change — the library list already carries narrators.
- **Unified library search across books, series, authors, and narrators.** The header search now matches your owned library on all four dimensions at once and groups the suggestions by kind (each linking straight to the book's detail page or the relevant collection page). Pressing Enter — or the new "See all results" row — opens a dedicated **search results page** (`/search`). The results page has tabs (with counts) for All / Books / Series / Authors / Narrators that double as a result summary and let you jump straight to one category; the **All** tab is a capped overview so a large Books match never buries the other categories (a "See all" link jumps to the rest). Each category offers the standard grid/list toggle and sort (Books by Title/Author/Year; collections by Name/book count), matching the library and collection pages. This is distinct from the existing "Add new" metadata search, which looks *outward* to Audible/indexers; this one searches what you already have. Entirely client-side over the already-loaded library, with the distinct-name index memoized so typing stays responsive.
- **Series detail page: sort by series order, and a clearer "Wrong series?" icon.** Series collection pages now offer a **Series Order** sort (and default to it) so books read in their in-series sequence — previously the series page hid the series sort entirely, leaving only Title/Author/Date Added. The ordering uses the existing zero-padded series-position key (handles decimal entries like 1.5; unnumbered/standalone entries sort last). The "Wrong series?" button's icon changed from a magnifying glass (easily confused with the adjacent "Search now") to a swap icon that better conveys re-selecting the correct series.
- **Library: the selected filter and sort are now remembered across visits.** Previously, returning to the library via the nav (a bare `/audiobooks` with no query) reset the dropdown filter and sort back to defaults — only in-app back/forward and shared links (which carry the state in the URL) preserved them. The last-used filter selection and per-group sort are now persisted to `localStorage` (mirroring the existing groupBy/viewMode persistence; not a cookie, so nothing rides on every request) and restored by seeding the URL on a plain visit. Shared links still win over the stored state, and dashboard drill-down links (`?missing=`, `?author=`, …) are never overridden by — or allowed to erase — the stored filter. Also fixed a latent bug where a reloaded `?sort=imported` (or `?sort=series`) URL silently fell back to title sort, because the init sort-hydration allow-list was out of sync with the actual sort options.
- **Library: new "Recently Imported" filter + sort — see the books you actually have, newest import first.** The built-in filter restricts the library to owned books (those with a file) and orders them by when their file was most recently imported, so freshly-added books surface at the top. Selecting it switches the sort to a new **Recently Imported** sort key (also usable on its own); books with no tracked import timestamp (e.g. legacy `filePath`-only records) sort to the end. The import time is `max(AudiobookFile.CreatedAt)` per book, surfaced on the slim library list via a new `ImportedAt` field (`IAudiobookFileRepository.GetMaxCreatedAtByAudiobookIdAsync`); it survives organize/rename (those mutate the file row in place) so reorganizing a book does not falsely mark it as just-imported. Distinct from the existing "Recently Added" filter, which approximates recency from publish year.
- **Activity page now shows where each download lives, explains "100% but still working" rows, and breaks down repeated attempts.** Each row carries a client chip (with a torrent/usenet/direct-download icon, tooltip "Downloading via &lt;name&gt; (torrent client)") so it's clear whether an item is on qBittorrent, NZBGet, a direct download, etc. — the `activity` endpoint now resolves the download client's implementation type (`ActivityItem.DownloadClientType`) alongside its name. The status badge gained a tooltip that explains each state and, crucially, why a row can read ~100% yet still show **Downloading** (bytes are complete but the client is still seeding / completion isn't detected) or **Processing** (download finished, files are being imported/moved) — the progress bar tracks downloaded bytes, the badge tracks pipeline stage. The "×N" attempt badge is now clickable and opens an **Attempt History** modal listing every grab attempt newest-first with its own status, timestamp, client, and failure reason, projected from the collapsed download group (`ActivityItem.Attempts`) so you can see what each retry was and why it failed.
- **Organize files: "Target file already exists" now opens a conflict resolver instead of dead-ending.** Re-tagging a book and organizing its files into the corrected folder used to fail with an unactionable *"One or more file organize operations failed."* banner whenever a file already sat at the destination — a soft, unlogged per-item error with no detail and no recourse (typically hit when two records point at the same book, e.g. a mis-identified duplicate organized into the correct book's folder). The backend now returns side-by-side conflict metadata for both files (size, format/codec, bitrate, duration, modified date) plus the audiobook record that *owns* the destination file when one is tracked (`RenameService` looks it up via the new `IAudiobookRepository.GetByFilePathAsync`), and logs the collision at Information so it's diagnosable. `RenamePreviewModal` surfaces a resolver — mirroring the duplicate-review flow — with an "open in new tab ↗" link to each book so you can preview the audio and metadata before deciding: **Overwrite** (delete the existing book/file, then move the incoming book into place), **Delete duplicate** (remove the book being organized and its files), or **Skip**. Destructive actions go through a per-card confirmation that names exactly which book and files will be deleted, and reuse the existing `DELETE /library/{id}` deletion path. A blind file overwrite is never performed when another record tracks the destination — that case is always resolved at the record level. The only new file-level operation, `FileRenameOperation.OnConflict = Overwrite`, is honored solely for an untracked orphan file on disk.
- **Download history is now pruned automatically (30-day retention).** The `Downloads` table — the download/import history — had no retention at all, so it grew unbounded on long-running instances (thousands of terminal rows). A new `DownloadHistoryRetentionService` background worker (mirroring the existing processing-job cleanup) runs ~5 minutes after startup and then daily, deleting terminal records — `Moved` / `Completed` / `Failed` / `ImportBlocked` — whose most recent activity (`CompletedAt`, else `StartedAt`) is older than 30 days. Active downloads (Queued / Downloading / Paused / Processing / Ready / ImportPending) are never touched regardless of age, and deleting a history row never affects the library (audiobook files live in their own tables/folders). Backed by a new `IDownloadRepository.DeleteTerminalOlderThanAsync`; unit-covered (old terminal rows purged, recent and active rows retained). The first run after deploy also flushes the accumulated backlog.
- **Activity page: summary chips + collapsed, dated, drill-down list.** A new `GET /api/downloads/activity` endpoint returns category counts (In progress, Blocked, and 24h Imported/Failed/Stalled) plus a per-book collapsed, newest-first list, computed server-side so the page no longer ships the entire download history (thousands of rows) to the browser. The Activity view now shows clickable count chips that filter the list by category, a "When" column with the activity timestamp and a relative age (timestamps were previously absent entirely), an "×N attempts" badge where repeated grabs for the same book are collapsed into one row, and a failure-reason breakdown when drilling into Failed/Stalled. The default view shows only what is actively in progress; blocked/imported/failed/stalled drop off until their chip is clicked (with the chip counts — Blocked stays all-time — keeping those backlogs visible). Categorization, collapsing, windowing, and counts live in a pure, unit-tested `ActivitySummary` helper (`Stalled` = a Failed download whose error marks a stall-timer reap; self-contained so it reads zero where no stall reaper runs).
- **"Search now" on a series — kick off a search for every missing or upgradable book in one click.** A new "Search now" action on the series page runs an on-demand automatic search for the series' owned, monitored books. It reuses the *exact* per-book logic of the background automatic-search cycle (via a new `IAutomaticSearchInvoker` on `AutomaticSearchService`, registered as a shared singleton), so it inherits the right gates: it skips a book that already has an active download or already meets its quality-profile cutoff, and only grabs a genuine upgrade — it will **not** re-download books you already have. It bypasses the 6-hour throttle (you asked for it now) since the books are selected explicitly. New `POST /search/now` takes a list of audiobook ids (de-duped, capped at 100 per request to protect indexers) and returns a per-book summary (`requested` / `queued` / `skipped` / `failed`); the FE surfaces that as a toast. Note: this inherits the background cycle's query construction, so its recall is only as good as that path (a separate query-normalization improvement is tracked).
- **A pinned series choice now sticks across syncs, and the "Wrong series?" picker has a manual fallback.** Two gaps in the series picker (#13): (1) confirming a series wrote the chosen ASIN only to the slug cache, so a *monitored* series re-resolved by name on its next sync — `SeriesMonitoringService.SyncSeriesInternalAsync` called `GetCatalogAsync(SeriesName, …)` and then overwrote `SeriesAsin` from that name-resolution, making a corrected ASIN inert. `MonitoredSeries` now carries an `AsinPinned` flag (additive, defaulted-false migration); when set, sync resolves the catalog via `GetCatalogByAsinAsync` (identical book population to the name path) and never lets name-resolution clobber the pinned value. New `PUT /series/monitoring/{id}` pins an ASIN and immediately re-syncs; the picker calls it after a pick so a monitored series stays pinned. If another monitored series already tracks that ASIN, the repoint **merges** into it (the redundant source row is deleted and the survivor pinned) rather than creating a duplicate monitor. (2) The picker offered no escape hatch when owned-book-derived candidates came back empty (the orphaned/renamed-series case — e.g. a "Smuggler's Tales" entry left behind as "A Smugglers Tale" with no catalog): it now has an editable **search box** to re-query Audible by a typed name and a **paste-ASIN** field to apply a known series ASIN directly. (Deleting such an orphan already worked via the existing Monitoring toggle.) See kevinheneveld/Listenarr#13.
- **"Add missing" on a series page — bulk-add the books you don't own.** A series page now shows an "Add missing (N)" action that opens a picker of the series' missing books (the page's already-deduped, ownership-correct works — no re-fetch, no duplicate editions, no books you already own under another series name). Uncheck any you don't want, choose root folder / quality profile / monitored, optionally "monitor the series", and add them in one pass; the library refreshes once at the end (not per book). "Already in library" (HTTP 409, e.g. a copy owned under a different series name) is treated as a benign skip, not an error. **"Search for downloads immediately" defaults OFF** so a bulk add doesn't fire dozens of indexer searches unless you opt in. See kevinheneveld/Listenarr#14.
- **Series/author pages collapse duplicate catalog editions into one entry per book.** Audible catalogs list the same title many times (regional/publisher re-releases, narrators) — the Jack Ryan chronological catalog returns 48 entries for ~32 actual books — which inflated the "total books" count and littered the grid with duplicate cards. The collection page now groups editions into logical *works* (key: normalized title with author/brand prefix, subtitle, and volume number stripped), shows one row per work, and filters out sub-series/bundle entries that masquerade as books (e.g. "Jack Ryan (chronological order) (abridged)"). The "total books", "in library", and "ready to add" counts all reflect works, not editions; ownership is matched against any edition of a work; and duplicate *library rows* for the same book collapse into one display entry too. The dedup is a deliberately good-enough default for messy ghost-written franchises — the eventual selection/curation UI is where residual near-duplicates get merged or excluded. (Representative-edition-by-preference, e.g. prefer unabridged, is a later refinement.) See kevinheneveld/Listenarr#14.
- **Series picker — correct a wrong or ambiguous series resolution from the series page.** A "Wrong series?" control (and a proactive low-confidence banner that appears when none of your owned books in the collection show up in the matched catalog) opens a picker listing candidate series: those derived from books you actually own are ranked first (and surfaced even when the real series name doesn't match the page's slug — e.g. two Forstchen books mis-tagged `After` resolve to their real `A John Matherson Novel` series, ranked above the unrelated Anna Todd "After" that a name search returns), followed by name-search matches. The best guess is pre-selected; confirming persists the choice (`POST /metadata/series/select` → resolve by the chosen ASIN, write it to the series cache keyed by the page's slug so it sticks and overwrites the prior wrong resolution) and re-renders the page. New `GET /metadata/series/candidates` backs the list. See kevinheneveld/Listenarr#13.
- **Series metadata resolves from a book you already own when a plain name lookup would fail or return the wrong series.** Scan-imported audiobooks often carry a series name that's mistyped or differently formatted from the canonical Audible spelling (e.g. a folder/filename gives `A Seekers Tale` for the canonical `Seeker's Tale`). A name-based series search on such a string doesn't just come up empty — it can return a *confidently-wrong* series with an ASIN (`Seekers Tale` → the unrelated "Seekers" by Erin Hunter), which then gets cached and served indefinitely. `SeriesCatalogService.ResolveSeriesAsync` now resolves in tiers on a cache miss: (1) a previously-resolved ASIN from the series cache; (2) **owned-book derivation** — find a library book in the series, look it up on Audible by its own ASIN (preferred) or title+author, read the embedded series ASIN whose *name matches* the target (so a book in multiple series can't resolve to the wrong one), and resolve the catalog by that ASIN; (3) name search as the fallback for series you don't own a book in. Owned-book derivation is preferred over the name search because it's grounded in a book you actually own, so it corrects wrong results, not just empty ones. The resolved series is persisted to the series cache so later loads short-circuit by ASIN. Owned-book probing is capped and the whole recovery is best-effort (failures log and fall through). Writing the recovered ASIN back onto the audiobook's `SeriesMembership` records is deliberately deferred to a metadata-rescan path rather than done from a read. See kevinheneveld/Listenarr#12.
- **Omnibus / anthology detection in the metadata backfill modal:** When the Audible record is meaningfully longer than the book's current runtime (3× or more), the modal surfaces a banner explaining that the match looks like a bundle and pre-unchecks `Title`, `Subtitle`, and `Runtime` so applying the change won't overwrite the story-specific values with the omnibus's. Cover, series, author, description, and the other shared-across-the-bundle fields stay defaulted ON so the typical "I have a story file, the only Audible record is the omnibus, I want to import everything *except* the title and runtime" workflow takes one click rather than a manual sweep through every checkbox.
- **Multi-region Audible search in the metadata backfill modal:** When no region is explicitly picked, the candidate search now walks US → UK → CA → AU, stopping at the first store that returns at least one match. Common case (US has it) stays a single request; titles only available in non-US stores (Audible UK in particular often carries omnibus editions and indie releases the US store doesn't) are surfaced automatically. Each candidate row shows a region chip (`US` / `UK` / `CA` / `AU`) so it's obvious where a match came from, and the subsequent per-ASIN metadata lookup is routed to the candidate's own region so a UK pick fetches its details from `audible.co.uk` rather than 404'ing against the US endpoint. A region selector dropdown next to the title/author inputs lets the user override the policy explicitly — pick `UK` and the modal queries UK only, no fallback.

### Performance
- **Download queue snapshot is no longer O(queue × tracked downloads).** `GET /download/queue` (and the background `QueueMonitorService`/`DownloadMonitorService` that call it on a timer) took 3.5–5s per snapshot on a large library — measured at ~278 live qBittorrent items × ~1,419 tracked candidates. The DB itself was fine (the whole `Downloads` table scans in ~8ms); the cost was `DownloadQueueService` scoring *every* candidate against *every* queue item, where each non-matching pair fell through `QueueItem.GetMatchScore` to `TitleUtils.NormalizeTitle` (seven regex passes) — millions of regex executions per cycle, every few seconds. A new `DownloadQueueMatcher` indexes the matching candidates once per snapshot by their identity keys (download id → score 4; `ClientDownloadId`/`TorrentHash` → score 3, keeping the latest `StartedAt` on collision so the old tiebreak is preserved), so the common case — a queue item whose torrent hash / client id matches a tracked download — resolves with an O(1) lookup and never normalizes a title. The title scan runs only as a fallback when no identity match exists, with the original ambiguity handling unchanged. This is provably equivalent to the old full scan (an identity match scores ≥3, a title match ≤2, so an identity hit is always the maximum), and is covered by a differential test asserting the indexed matcher selects the identical `Download` as the reference scan across every match tier. The residual fallback (queue items added outside Listenarr, which have no identity match) is further cut by memoizing `TitleUtils.NormalizeTitle` — previously re-running its seven regex passes on the same candidate titles for every unmatched queue item; the cache is bounded and, being pure, is process-wide safe. Reconcile/orphan-retention behavior is unchanged — it is derived from the same per-item selection. Measured live: ~3.5–5s → ~1.6s per snapshot.

### Fixed
- **Music-scene releases that match an audiobook title by chance are no longer grabbed.** `AudiobookOnlyFilter` rejected comics/ebooks/video by file extension, but music files are `.mp3`/`.flac`/`.m4a` — the same extensions audiobooks use — so a music single like `Calhem Parker and Ola Rostrom-Clash of the Titans-[IVT075]-SINGLE-WEB-2026-PTC` matched the generic audiobook title "Titans," passed the filter, and was grabbed and imported as the wrong content. The filter now also rejects releases whose title/format carries a music-scene format-pair — a release-type token (`SINGLE`/`EP`/`ALBUM`/`CDM`/`CDS`/`VINYL`/…) immediately followed by a medium (`WEB`/`CD`/`VINYL`/`FLAC`/…), e.g. `SINGLE-WEB`, `EP-CD`, `CDM-FLAC` — when there is no positive audio signal (runtime, narrator, or trusted audio metadata source). Kept deliberately to the *delimited pair* rather than bare `SINGLE`/`WEB` so ordinary titles ("Single Malt Murder") and audiobook web rips are not caught. Runs in every search path's selection pass, so it stops the bad grab for background, failed-retry, and manual searches alike. Unit-covered (the live `SINGLE-WEB` case and `EP`/`ALBUM`/`CDM` variants filter; plain M4B/MP3 audiobooks, a "Single Malt" title, and a bare "WEB Edition" do not).
- **Successful NZBGet (usenet) downloads are no longer orphaned in "Queued" forever and now import.** A finished NZBGet download leaves the active queue (`listgroups`) and moves to history, but the only path that ever marked an NZBGet download `Completed` ran inside the `listgroups` poll loop (`AdapterUtils.MapDownloadProgress`) — which by definition can only see items *still in the queue*. A small audiobook can be grabbed, downloaded, post-processed, and moved to history all within one 30-second poll window, so its terminal state was never observed and the record sat in `Queued` indefinitely; the history-reconciliation pass that catches dupe-deleted/failed grabs explicitly skipped `SUCCESS/*`, deferring to that unreachable completion path. With automatic search on, the stranded book was then re-grabbed, and NZBGet rejected the re-grab as a duplicate of the already-completed original (`DELETED/COPY`), so it import-blocked — a loop that left a live instance with **zero** NZBGet imports ever recorded despite hundreds of successful downloads. `ReconcileTerminalFromHistoryAsync` now also completes `SUCCESS/*` history entries: it points the download's `DownloadPath` at the history `FinalDir` (falling back to `DestDir`; the gateway then applies any remote-path mapping) so the processing job can locate the files, then transitions the record to `Completed`, which fires the normal import handoff. (The NZBGet adapter, unlike qBittorrent/SABnzbd, never set `DownloadPath` during polling, so even a caught-in-queue completion previously reached the processor with an empty path and could not import — this fixes that too.) Unit-covered (`ReadHistoryDir` prefers `FinalDir`, falls back to `DestDir`, and returns null when neither is usable).
...
- **Activity page no longer pegs CPU while the tab is open.** The Activity view polled `apiService.getQueue()` every 30 seconds, and that endpoint runs the full `DownloadQueueService` pipeline on every call — path-translating the live queue, metadata-enriching ~1,000+ tracked downloads, and reconciling records missing from the client snapshot — which took 10–30 seconds and ran the whole time the tab stayed open. The poll was redundant: live progress for in-progress rows already arrives via the SignalR `QueueUpdate` subscription the view holds, and `getActivity()` (≈0.2s) carries database progress. `refreshQueue()` no longer calls `getQueue()`, so opening the Activity tab no longer triggers the expensive pipeline on a timer.
- **A single malformed qBittorrent queue field no longer aborts the entire queue fetch.** `QbittorrentAdapter.GetQueueAsync` read numeric `torrents/info` fields with the strict `JsonElement.GetInt32/GetInt64/GetDouble` accessors, which throw `System.FormatException` when a value is out of the .NET type's range (e.g. an `eta` beyond `Int32`) or, across qB versions, string-encoded. One such field made the whole fetch fail with "client may be unreachable", emptying the queue snapshot. (The N+1 fix below exposed this by letting the fetch run to completion instead of timing out first.) The numeric reads now use tolerant helpers (`ReadJsonInt32/Int64/Double/String`) that clamp out-of-range values, accept string-encoded numbers, and fall back instead of throwing. Unit-covered.
- **qBittorrent queue polling no longer fires one `/torrents/files` request per torrent every cycle (N+1), which was timing out the WebUI and pegging the host.** `QbittorrentAdapter.GetQueueAsync` builds the queue/monitoring snapshot every poll cycle (from several background services) across *every* torrent in the configured category — including the hundreds of completed/blocked torrents that are still seeding. For each one it made a separate `GET /api/v2/torrents/files?hash=…` call purely to resolve the content path and file list. On an instance with a large backlog that's 1,000+ extra HTTP requests per cycle, which overwhelmed qBittorrent's WebUI (surfacing as `Using cached queue snapshot … reason: timeout` and the "cached data after a client error" banner) and drove host load. The snapshot now reads the path directly from the `content_path` field already returned by the single bulk `torrents/info` request (the same field the import path already relies on — `GetImportItemAsync` documents "ContentPath/OutputPath is already set correctly from content_path field"), and leaves the per-file list empty. The N+1 collapses to one request per cycle. The per-file list is still enumerated **lazily, only for the one item being imported**, via `DownloadItemService.GetImportItemAsync`. To keep this from simply moving the cost to the filesystem, `DownloadClientGateway.TranslateQueueItemPathsAsync` gained an `enumerateSourceFiles` flag: the bulk snapshot path passes `false` (path translation only — no recursive `Directory.EnumerateFiles` walk of the download directory per item per cycle), while the import path keeps the full enumeration. Unit-covered (the bulk path leaves SourceFiles empty even when the content directory has files; the import path still scans it).
- **Activity page: rows no longer flip between "Import Blocked" and "Downloading"/"Completed".** The list previously built each row by merging the live download-client queue snapshot with the database record and letting whichever source refreshed last win the status — so a finished-but-import-blocked torrent that is still seeding in the client kept toggling the row between its live "downloading" state and its stable `ImportBlocked` database status. The view now takes status from the authoritative database record (the activity endpoint) and uses the live queue snapshot only to overlay download progress/ETA onto in-progress rows, so the displayed status is stable.
- **A comic/ebook/video release whose title happens to match an audiobook is no longer grabbed — the audiobook-only filter now rejects non-audio file formats.** `AudiobookOnlyFilter` rejected print/box-set product pages (Paperback, Hardcover, eBook, Kindle, Box Set, …) but had no entry for non-audio *media* formats, so a comic like `Tom Corbett Space Cadet V2 001(2013)(Digital)(TLK EMPIRE HD).cbr ( Nem )` — title-relevant to the audiobook, no audio signal — passed the filter, scored, and was grabbed. It then either import-blocked (no audio inside) or stalled as a dead torrent, and automatic search re-grabbed it, looping `Downloading → Import Blocked/Failed`. The filter now also rejects results carrying a comic (`.cbr/.cbz/.cb7`), ebook (`.epub/.mobi/.azw3/.azw`), or video (`.mkv/.mp4/.m4v/.avi`) file extension when there's no positive audio signal (runtime, narrator, or trusted audio metadata source). Matched as dotted extensions so audiobook formats (`.m4b/.mp3/.m4a/.flac`) never collide and a legitimate "M4B + PDF" bundle is not caught (bare `PDF` is deliberately omitted). Because the filter runs in every search path's selection pass, this stops the bad grab for the background cycle, the failed-download retry, and manual search alike. Unit-covered (the `.cbr` case filters; an `M4B + PDF` audiobook and a plain MP3 release do not; a result with an explicit runtime is treated as audio regardless of extension). A book that has *only* a comic/print match on the configured indexers will now correctly sit wanted-but-unfulfilled rather than churn-grabbing the wrong file.
- **Successful and failed imports are recorded in download history again, so the dashboard "Imported" graph and import success-rate stat stop reading zero.** The import-pipeline rationalization (#535/#492) moved completion handling onto `DownloadProcessingJobProcessor` but left the `Imported`/`ImportFailed` `DownloadHistory` event recording behind in the now-orphaned `DownloadValidationPipeline` (no live callers). As a result no `DownloadHistoryEventType.Imported` row had been written since that refactor deployed, even though downloads kept reaching `Moved`: the dashboard Activity chart's "Imported" series — and `LibraryStatsRepository`'s `TotalImports`/`ImportSuccessRate`, all of which query `DownloadHistories` — flatlined at zero while the library was in fact growing. The processor now records the event at its two terminal funnels: `IDownloadHistoryService.RecordImportedAsync` right after `download.Imported()` on the success path, and `RecordImportFailedAsync` at the single block-the-download funnel where a job that has exhausted its retries lands. (`audiobookId` is passed null — `Download.AudiobookId` is an int key while the history column is a Guid, and the metric only counts events.) Unit-covered (success records Imported once; terminal failure records ImportFailed once). Restores recording going forward; it does not backfill imports that completed without a history row during the regression window.
- **NZBGet downloads it deleted as duplicates (or that failed) are no longer orphaned in `Queued` forever — the adapter now reconciles against NZBGet history like the SABnzbd and qBittorrent adapters do.** `NzbgetAdapter.FetchDownloadsAsync` polled only the active queue (`listgroups`) for progress; when NZBGet finished with an item it leaves the queue and moves to history, so a grab NZBGet's own dupe-check removed (`DELETED/COPY`, `DELETED/GOOD` — on the live instance, 148 of 501 history items, a direct readout of the duplicate-edition problem) or that failed (`FAILURE/*`) never got a terminal state and sat in `Queued`/`Downloading` indefinitely (92 stuck on the live box). The poll now runs a history-reconciliation pass: for tracked downloads still non-terminal **and absent from the queue this cycle** (guarded so it only runs when the queue fetch succeeded — "absent" must be meaningful), it matches NZBGet history by NZBID (title-similarity fallback) and resolves the terminal state. Duplicate removals (`DELETED/COPY|DUPE|GOOD`, and ambiguous `DELETED/MANUAL|SCAN`) are **blocked** (`ImportBlocked`) rather than failed, so they don't trip the `FailedDownloadAutoSearch` re-grab loop that would just re-fetch a duplicate NZBGet rejects again; genuine failures (`FAILURE/*`, `DELETED/HEALTH|BAD`) are **failed** so the normal retry/auto-search applies. `SUCCESS/*` is left to the completion-detection path. Because `ImportBlocked` isn't a valid transition from `Queued`, a queued-but-removed dupe is nudged through `Downloading` first (only the terminal state is persisted). The classification policy is an `internal static` helper (`ClassifyNzbgetHistoryStatus`), unit-covered across the duplicate, real-failure, and success/non-terminal cases. (Prevents *new* orphans and reconciles ones it can match live; the existing stuck backlog and the upstream duplicate-grabbing cause are separate cleanups.)
- **The background automatic search no longer re-grabs a book whose import is `ImportBlocked` — but the manual "Search now" still can.** A blocked import (e.g. the NZBGet duplicate above, or a duplicate-edition collision) means the book needs attention, not another grab; the background cycle previously treated only `Queued`/`Downloading`/`Paused`/`Processing`/`Ready`/`ImportPending` as "already has a download, skip" and left `ImportBlocked` eligible, so it re-searched and re-grabbed on the next cycle — the very churn the NZBGet block was meant to stop. `AutomaticSearchService.ProcessAudiobookAsync` now also skips a book whose only download is `ImportBlocked`, gated by a `suppressIfImportBlocked` flag the background cycle passes `true`. The on-demand "Search now" path passes `false`, so an explicit user request still overrides the block and searches. Unit-covered (manual override searches; active-download skip and cutoff-met skip unchanged). Affects all `ImportBlocked` downloads, not just NZBGet duplicates — a blocked import is no longer auto-retried until it's unblocked, re-searched manually, or the download is removed.
- **The scan no longer broadens a per-book BasePath back to the library root or an author folder.** `ScanBackgroundService` derives an audiobook's `BasePath` from `CalculateBasePath(foundFiles)` — the longest common ancestor of the scan-matched files. When a scan's match window spanned multiple books in the same author folder (the common case for the broad-BasePath rows the kevin/live import-pipeline fixes address), the LCA collapsed to author level or even to the bare library root (`/audiobooks`), and the scan adopted that as the audiobook's BasePath — re-introducing the broad-BasePath → cross-edition import collision → wasted-bandwidth loop that the earlier `LibraryAddService`/`DownloadImportService` fixes only stopped at *add* and *import* time, not at *scan* time. The scan now refuses to overwrite BasePath when (a) the LCA equals any configured library root folder, or (b) the LCA is a strict ancestor of an already-set non-root BasePath. The acceptance check is a `public static` helper (`IsAcceptableScannedBasePath`) so it is unit-covered (12 cases: root equality with/without trailing slash, author-level broadening, root-level even-when-current-is-per-book, accept on null current, accept on root current, accept on deeper/equal-to-current refinement, null/empty candidate, null/empty roots collection, multi-root configurations, trailing-separator handling). Refinements to a deeper per-book path still take effect. (Companion to the planned data migration that re-derives per-book BasePaths for the existing ~927 broad/empty rows; this fixes the *second* code source of broad BasePaths that the `LibraryAddService` fix did not cover.)
- **Duplicate-edition downloads (same torrent re-grabbed for a sibling record) now fail fast with a clear collision message instead of running 3 retries to ImportBlocked under a misleading "files reported by the download client and files on disk do not match" message.** When the same torrent hash is grabbed for multiple audiobook records (different editions of the same work), the first job moves the audio into the owning record's library folder (`CompletedFileAction=Move`); subsequent sibling jobs find only the companion files the torrent client still reports (covers, PDFs, `.nfo`) and trip the strict "client reported N files / N-k actually on disk" mismatch. On the live instance this was the dominant remaining ImportBlocked-with-real-data class: 107 of 107 such failures involved a same-hash sibling, and 70% of those siblings had already reached `Moved` (the audio physically went into the library under another audiobook record). The brief's prescribed Mode-1 fix ("import the audio files that are present") doesn't fit: in 70% of cases the only "audio" left on disk is the cover/PDF that came with the pack, and importing those as audiobook files would be actively wrong. `DownloadProcessingJobProcessor` now consults a new `IDownloadRepository.GetByTorrentHashAsync(hash, excludeDownloadId)` at the mismatch branch — if any same-hash sibling is at `DownloadStatus.Moved`, the job fails immediately via `MarkAsFailed` with "Duplicate-edition collision: this torrent's audio was already imported for audiobook N; only companion files (covers/PDFs) remain on disk for this record. Consider deduplicating these audiobook records." When siblings exist but none have yet imported (the 30% race-or-batch case), the existing retry path is preserved so a download that might still legitimately import is not preemptively blocked. Usenet downloads (no `TorrentHash`) bypass the new check entirely. The lookup walks the Downloads table in memory (mirroring `GetKnownClientItemIdsAsync`'s existing pattern) because EF Core can't cleanly translate `json_extract` over SQLite without raw SQL; the table is small enough that the cost is negligible relative to skipping three retry cycles. Unit-covered (the moved-sibling case fails fast with the collision message; the no-moved-sibling case still schedules a retry).
- **SQLite "database is locked" (SQLITE_BUSY) errors from concurrent background services are now absorbed instead of failing the cycle.** The connection string was a bare `Data Source={path}` with no `busy_timeout`, so when two of the singleton-`IDbContextFactory`-backed background services (`DownloadMonitorService`, `QueueMonitorService`, `MovedDownloadProcessor`, `StalledDownloadReaper`, the scan services) tried to write at once, the loser got `SQLITE_BUSY` *immediately* — surfacing as `Microsoft.Data.Sqlite.SqliteException: SQLite Error 5: 'database is locked'` (and the related "unable to delete/modify collation sequence due to active statements"), which the sibling fix above merely stopped from being fatal. A new `SqliteConnectionPragmaInterceptor` (a `DbConnectionInterceptor` wired into the factory's `DbContextOptions` in `AddListenarrInfrastructure`) now runs `PRAGMA busy_timeout=5000;` on **every** connection EF opens — `busy_timeout` is per-connection and non-persistent, so it must be re-applied on each pooled open, which an interceptor guarantees and a connection-string keyword can't (Microsoft.Data.Sqlite's `Default Timeout` is the *command* timeout, not SQLite's busy handler). A writer now waits up to 5s for the lock to clear before giving up, which is far longer than these sub-second overlapping writes need. The interceptor also re-asserts `journal_mode=WAL`, `synchronous=NORMAL`, and `journal_size_limit=6144000` (idempotent no-ops once set) so a freshly created DB gets the right pragmas without relying on state persisted in an existing DB file; it's a no-op for non-SQLite providers (the InMemory test path). This also replaces the dead `SqlitePragmaInitializer` (defined but never called anywhere), which is removed. Covered by a test that resolves `IDbContextFactory<ListenArrDbContext>` from the real DI registration, opens a connection through EF, and asserts `PRAGMA busy_timeout` reports 5000.
- **The whole app restarted every ~6 minutes (host crash-loop) whenever a download-monitor cycle hit a transient SQLite "database is locked".** `DownloadMonitorService`'s poll loop caught only `OperationCanceledException`, so any other exception thrown by a cycle — in practice a `Microsoft.Data.Sqlite.SqliteException` ("database is locked" / "unable to delete/modify collation sequence due to active statements") raised while persisting a download via `EfDownloadRepository.UpdateAsync`/`GetByIdAsync` — propagated out of `ExecuteAsync`. An unhandled exception out of a `BackgroundService` trips the .NET host's default `BackgroundServiceExceptionBehavior.StopHost`, so the host shut down gracefully (exit 0) and the container's `unless-stopped` policy restarted it — repeating every poll cycle (~18 restarts in 9h on the live instance). Each restart re-ran startup scans (the ffprobe re-probe flood) and **reset the stall-reaper's in-memory 60-minute timers so it never reaped**, keeping CPU pegged. The other background services (`QueueMonitorService`, `MovedDownloadProcessor`, `AutomaticSearchService`) already log-and-continue on a failed cycle; `DownloadMonitorService` was the lone exception. The poll loop (extracted to `RunMonitorLoopAsync` for isolation/testability) now swallows-and-continues on any non-cancellation, non-fatal exception (logging "Error in Download Monitor Service") so a transient lock no longer takes down the app. Unit-covered. (The underlying SQLite write contention — the connection string sets no `busy_timeout` — is a separate hardening follow-up; this stops a single transient error from being fatal.)
- **An import refused because its files already belong to another audiobook now reports that distinctly, instead of the misleading "No audio files were registered … but files have been imported".** When the overwrite guard skips every file of a download because each destination is already owned by a sibling record (a duplicate/edition collision), the job now fails with "All files already belong to another audiobook (likely a duplicate or edition-record collision) …" rather than the generic registration-gap message — so the real cause (records that should be deduplicated) is visible in the job error and findable by a future dedup pass. `ImportResult` gained a typed `SkippedDueToOwnershipConflict` flag (set by the guard) so the processor distinguishes the two cases without string-matching. Unit-covered.
- **A completed download for an audiobook with no `BasePath` hard-failed the whole import instead of deriving a folder.** `DownloadImportService.ImportDownloadFilesAsync` threw `InvalidOperationException("…basePath cannot be empty or null")` when an audiobook reached import with an empty `BasePath` (scan-added or partially-populated rows), sending the download to `ImportBlocked` even though the files were sitting on disk ready to import. It now derives a per-book folder from the configured default root folder + `FolderNamingPattern` (the same shape `LibraryAddService` uses), guarded so an empty pattern result falls back to the sanitized title/ASIN rather than the bare root; it only fails if nothing usable can be derived. Unit-covered.
- **Adding a book stored the bare library root as its `BasePath`, so different books/editions shared one folder and collided at import.** The "add to library" UI sends the selected *root folder* as `DestinationPath`, and `LibraryAddService` stored it verbatim as `BasePath` — only deriving a per-book folder (`{Author}/{Series}/{Title}/…`) when no destination was supplied. So books added this way got `BasePath = /audiobooks` (or an author-level folder), and at import every one of them computed the same flat filenames in that shared folder; the second arrival's files landed on a path the first already owned (→ overwrite/registration-collision/re-download loop). `LibraryAddService` now anchors a per-book folder under the root whenever `DestinationPath` is one of the configured root folders (and still does so when it's empty); a genuinely specific per-book `DestinationPath` is still used as-is. Guarded so an empty naming-pattern result (poor metadata) falls back to the sanitized title/ASIN rather than the bare root. Unit-covered. (Fixes the *source* of new broad-BasePath rows; cleaning up the existing rows is a separate data migration.)
- **Import could overwrite a file already owned by a *different* audiobook, corrupting a working book and then looping.** `FileMover` moves/copies with `overwrite: true`, and the move happens *before* registration. When a redundant or edition-colliding download resolved to a destination path another audiobook already owned (common when multiple edition records share a broad/author-level/root `BasePath`, so different downloads compute identical flat filenames), the import silently overwrote the owner's library file — then `EnsureAudiobookFileAsync` refused to register it ("already registered to another audiobook"), so the download never satisfied its book and got re-grabbed. `DownloadImportService` now checks, *before* each move (audio and companion files), whether the destination path is registered to another audiobook (`IAudiobookFileRepository.IsPathUsedByOtherAsync`) — regardless of whether the file is currently on disk, since the common live state is row-present/file-moved and letting the move proceed there would re-create the cross-attribution; if so it skips that file with a clear "refusing to write" result instead of clobbering the owner. The colliding download fails cleanly without data loss. Unit-covered. (Diagnosed live: ImportBlocked-with-real-data downloads whose audio had already been moved into the library under a sibling edition record — e.g. a "[Tantor]" grab attached to one *Tale of Two Cities* record writing onto paths owned by another.)
- **Stall-reaper was deleting usenet downloads that were merely *waiting* in NZBGet's queue.** NZBGet processes its queue serially, so with a deep audiobook queue (and other apps' movie/music downloads ahead) a freshly-grabbed NZB can sit `QUEUED` in NZBGet for well over an hour before its turn — making "no download progress" *legitimately*. The reaper's stall timer couldn't tell waiting-in-queue from stalled-dead, so it transitioned those items to `Failed` **and removed them from NZBGet**, deleting perfectly good downloads before they ever started. Live impact: in one 4-hour window **74 real, in-NZBGet usenet downloads were reaped** (vs 5 dead ghosts) and **0 usenet imports completed**, even though grabs were landing in NZBGet correctly. `StalledDownloadReaper.Evaluate` now takes the client-reported state and **never reaps an item the client reports as queued** (`QUEUED`/`PP_QUEUED` for NZBGet/SABnzbd, `queuedDL`/`queuedUP` for qBittorrent — see `IsClientQueued`); the stall snapshot is kept fresh so the timer only starts once the item is genuinely transferring. Torrents and truly-stalled states (`metaDL`, `stalledDL`, unmatched ghosts) are still reaped as before. Unit-covered.
- **Usenet (NZBGet) downloads silently stopped working: every grab stranded in `Queued`, never sent to the client.** Two independent bugs in the NZBGet path, both surfaced on the live instance (≈364 usenet grabs stuck `Queued` with no client id, while the search side was fine — NZBgeek returns audiobooks normally; the perceived "usenet finds nothing" was the now-fixed over-specified-query bug). **(1) Cross-host redirect block (the dominant one).** `DownloadNzbAsync` fetched the NZB through the named `nzbget` HttpClient, whose `NzbgetSafeRedirectHandler` refuses *all* cross-host redirects (it exists to keep NZBGet API Basic-auth same-origin). But the NZB lives on the *indexer*, and indexer download URLs routinely 301-redirect cross-host (e.g. Prowlarr → `nzbgeek.info`), so every fetch threw, the upload to NZBGet never happened, and the download sat in `Queued` forever (a regression dated to when the safe-redirect handler landed). `DownloadNzbAsync` now follows redirects itself — cross-host allowed, HTTPS→HTTP downgrade still implicitly safe because the api key rides in the URL query (no header credential to leak), SSRF re-validated on *every* hop, 10-hop cap — mirroring the existing `TorrentFileDownloader`. The NZBGet *API* client keeps `NzbgetSafeRedirectHandler` unchanged (those calls don't redirect cross-host). **(2) #619 — progress/completion never updated.** `FetchDownloadsAsync` read `FileSizeMB`/`RemainingSizeMB`/`NZBID` from NZBGet's JSON-RPC with `JsonElement.GetString()`, but NZBGet sends them as JSON *numbers* — `GetString()` throws on a number, so every group threw (`"Error updating NZBGet queue progress for group"`, flooding the log) and no usenet download's progress or completion was ever recorded. New `NzbgetAdapter.ReadJsonNumber` reads either number or string shape without throwing; unit-covered. See Listenarrs/Listenarr#619. (Both needed for end-to-end usenet: (1) gets the grab into NZBGet, (2) lets it complete and import.)
- **Stalled/dead downloads were never reaped, so the poll loop re-enriched and content-path-scanned hundreds of zombie torrents every cycle (the live CPU driver).** Once the metaDL completion fix below stopped falsely completing dead torrents, they correctly stayed `Downloading`/`Queued` — but *forever*, because nothing removed them. On the live instance that left ~495 `Queued` + ~226 `Downloading` rows (only ~13 genuinely progressing; the rest 0-seeder `stalledDL`/`metaDL`, plus a large set of "ghost" rows with no matching client torrent at all), each re-enriched and path-scanned on every poll — load ~37, Listenarr at 305% CPU. New **stall-timeout reaper** in `DownloadMonitorService`: it keeps a per-download progress snapshot and, when a `Queued`/`Downloading` row makes **no** download progress for a configurable timeout, transitions it to `Failed` (which excludes it from `GetActiveAsync` and both queue-candidate queries — i.e. from every recurring enrich/scan loop), removes it from the download client, and records a history entry. Detection is a progress-*stall* timer rather than a `progress==0`/`StartedAt` heuristic: live data showed the dead set sits at fractional progress (not zero) and includes unmatched ghosts, and any genuine byte of progress resets the timer so a slow-but-alive download is never reaped. The pure decision logic is `StalledDownloadReaper.Evaluate(...)` with full unit coverage. **Destructive, so it ships disabled and dry-run-by-default**, gated on environment variables (no schema migration): `LISTENARR_STALL_REAPER_ENABLED` (default `false`), `LISTENARR_STALL_REAPER_DRY_RUN` (default `true` — logs `[STALL-REAPER][DRY-RUN] Would reap N …` with sample titles but removes nothing), `LISTENARR_STALL_REAPER_TIMEOUT_MINUTES` (default `60`), and `LISTENARR_STALL_REAPER_DELETE_FILES` (default `false` — just drop the torrent/nzb from the client and leave any partial files for a later disk sweep; set `true` to also delete the partials). The dry-run report lists, per sampled row, its last-known client state (`stalledDL`/`metaDL`/`queuedDL`/`unmatched`), progress, and age, so a recoverable `queuedDL` is easy to eyeball before arming. The reaper is **client-agnostic** (it runs in the shared monitor loop over `Download` fields, so it also reaps stalled NZBGet/Transmission rows — on the live instance the largest dead population, ~408 `Queued`, was NZBGet ghosts, not qBittorrent) and deliberately does **not** auto-search after reaping, so a dead download isn't immediately re-grabbed. (A complementary grab-time seeder filter — refusing to queue 0-seeder results in the first place — is a separate follow-up.)
- **qBittorrent downloads that never started (dead/0-seeder magnets) were falsely marked complete, then piled up as `ImportBlocked`.** The qBittorrent poller's completion check was `progress >= 1.0 || amount_left == 0`. A torrent stuck in `metaDL` — still fetching its `.torrent` metadata, typical of a magnet with zero seeders — reports `amount_left == 0` because its total size isn't known yet, *not* because it finished. So the poller marked it `Completed`, enqueued an import job, the client reported **0 files**, and after exhausting retries the download landed in `ImportBlocked` ("Unable to import the download"). On the live instance this was the dominant import failure (≈595 `ImportBlocked` rows; ≈895 jobs failing "Max retries exceeded → No importable files found"), and it grew once the automatic-search recall fix below started grabbing many more (often dead public-tracker) torrents. Completion now requires `progress >= 1.0 || (amount_left == 0 && size > 0)` — the known-size guard excludes `metaDL`, and since `progress >= 1.0` independently catches every genuine completion the change can only make detection stricter, never miss a real finish. Extracted as a pure `QBittorrentHelpers.IsTorrentComplete(progress, amountLeft, size)` with unit coverage. (Transmission's poller already required `percent >= 1.0 && seeding-status`, so it was unaffected.) Also fixed the block message, which referenced the failing job's id via a non-interpolated string (`"…job {job.Id}…"`) and so lost the one breadcrumb pointing at the job log.
- **Automatic search found nothing because the query was massively over-specified.** Both automatic-search query builders (`AutomaticSearchService` for the background cycle + per-series "Search now", and `DownloadService.SearchAndDownloadAsync`) built the query as raw `Title + author [+ series]` — keeping the title's volume suffix (`, Book 4`), edition tags (`(Unabridged)` / `(Excerpt)`), and a series name that's usually already inside the title. The result, e.g. `Harry Potter and the Goblet of Fire, Book 4 J.K. Rowling Harry Potter`, AND-token-matched against nothing on indexers — observed live returning **0 results for Harry Potter**. New shared `SearchQueryBuilder` produces a clean `"<title> <author>"` query: strips volume markers (`Book N` / `Vol. N` / `Part N` / `#N`), strips parenthetical/bracketed edition tags (reusing `TitleUtils.NormalizeTitle`), drops the redundant series, and omits an author already present in the title. Both search paths also gain a **title-only fallback**: if the primary query returns nothing, they retry once without the author (covering releases whose author string differs from ours). This is the bottleneck fix for *all* automatic downloading, not just "Search now".
- **A series/author book you already own but filed under a different series name showed as "Not Added", and trying to add it failed with a 409 "Audiobook already exists".** The collection page deduped the external catalog only against library books filed under that page's exact series slug, so a book owned under a differently-spelled series (common with split/mis-parsed series, and now visible via the series picker, #13) appeared addable — and the add was correctly rejected by the backend. The catalog merge now dedups against the **whole library** by ASIN/ISBN (then title+author), so an owned book is recognized regardless of which series name it's filed under and renders in-library instead of offering a broken Add. (The underlying data split is addressed separately by #15.)
- **Persisted author/series catalog cache was silently dead on SQLite (production), re-fetching from Audible on every collection-page load.** The cache read queries ordered by `entry.LastFetchedAt.GetValueOrDefault(entry.UpdatedAt)`, which the SQLite EF provider cannot translate — it threw `InvalidOperationException` at runtime, and the caller's best-effort `catch` swallowed it and returned no cache hit. So `GetCachedAuthorByNameAsync`/`ByAsin` and `GetCachedSeriesByNameAsync`/`ByAsin` never returned anything, and every author/series page re-resolved against Audible. Rewritten to `entry.LastFetchedAt ?? entry.UpdatedAt` (translates to SQL `COALESCE`). Besides the obvious request-count/latency win, this also unblocks the series picker (#13): the user's chosen series is now actually served back from the cache instead of being re-resolved (and re-clobbered) on the next load. Caught by adding real-SQLite repository/service tests — the previous tests used the EF InMemory provider, which tolerates this untranslatable LINQ.
- **Series collection page no longer dead-ends on "Error Loading Library" when external metadata is missing or the URL slug doesn't match the stored name.** A series reached via a slug that differs from the canonical stored name (e.g. `A Seekers Tale` parsed from a filename vs. the stored `Seeker's Tale`) failed twice over: the collection matcher treated the possessive apostrophe (`Seeker's` → `seeker s`) and a leading article (`A `) as significant, so no library book matched and the page looked empty; and `getSeriesCatalog`/`getAuthorCatalog` swallowed every error with `catch { return null }`, so a legitimate `404 Series not found` from the metadata provider rendered as a generic, detail-free "Error Loading Library" card with a Retry button. The catalog API methods now treat a 404 as a legitimate "not found" (return `null`) and rethrow every other error so genuine failures surface real detail. `CollectionView` adds a series-only `normalizeSeriesName` (collapses possessive apostrophes, strips a leading `A`/`An`/`The`; author/narrator matching is left unchanged since a leading "A." initial is significant there) and a `resolvedSeriesName` that derives the canonical series name from a matching library book — used for the catalog/lookup/monitoring calls and the header instead of the raw slug. A 404 with no matching library books now shows the friendly empty state rather than an error. See kevinheneveld/Listenarr#11.
...
### Added
- **Organize library folders (Library Maintenance):** Library-wide maintenance action that walks every audiobook and moves each one into its canonical `{Author}/{Title}/` folder using the configured `FolderNamingPattern`. New `GET /library/organize/preview` is read-only and buckets every row as `will_move`, `already_canonical`, `collision` (≥2 audiobooks compute to the same target path — e.g. edition-variant duplicates the same-ASIN dedup tool wouldn't catch), or `invalid_target` (missing Author/Title, no configured root, empty pattern result). New `POST /library/organize/apply` takes the user-confirmed id list, re-validates each id against live DB state, drops ids whose target collides with another id in the same selection or whose `BasePath` already matches the canonical target, then queues per-book moves through the existing `MoveQueueService` (same safety checks as the per-book Organize button — no new filesystem code). Failures don't abort the rest; the response reports `queued`, `skipped`, `failedToQueue` counts plus a warning list. Mismatching ids (rows that no longer exist) return 400 without queuing anything. Surfaced in Settings → Library Maintenance via an "Organize library…" button that opens a modal showing bucket counts, a will-move list with per-row checkboxes (default-on; total file count + bytes), an explicit collisions section grouped by target path, an invalid-target section with the per-row reason, a confirmation panel before Apply, and a toast on completion.
- **Duplicates modal — narrator chip + full-delete warning for title/author groups:** Every row in the duplicates modal now shows a narrator chip (🎙) — when rows in the same group disagree about their narrators, the chip turns orange with an outline to flag the case. Narrator isn't used in the search-query construction, the relevance filter, or the match-confidence score anywhere in the pipeline, so two rows with the same author+title but different narrators are effectively the same to Listenarr's automated matching — the chip is the only place that surfaces the human-meaningful difference. The confirmation panel grows a loud red warning when a title/author group has every row marked Discard, naming each book ("Listenarr will stop tracking and searching for: *Bands of Mourning*, *Hero of Ages* …") and reminding the user to leave at least one row on Skip to keep monitoring. Help text in the title/author tab now walks the user through the right consolidation pattern ("pick Discard on the duplicates; leave at least one on Skip to keep being monitored").
- **Find duplicate audiobooks — title/author collision pass:** `GET /library/duplicates` now runs a second detection pass alongside the same-ASIN one. Rows that compute to the same canonical folder target via the configured `FolderNamingPattern` (the same `{Author}/{Title}/…` path the organize-library preview produces) but have distinct ASINs are surfaced as `kind: "title_author"` groups, with a `collisionKey` (the shared normalized target path) on each group. Sequential resolution: a row already in a same-ASIN group is excluded from this pass — after the ASIN merge runs, a re-fetch surfaces any residual collisions involving the survivor — so every row lives in exactly one group at a time. The response now includes `asinGroupCount` and `titleAuthorGroupCount` alongside `totalGroups`. **Phantom-row filter:** within a title/author group, rows with zero tracked `AudiobookFiles` are dropped when at least one row in the group has files — a monitored-but-not-downloaded record sitting at the same canonical target as a real owned book is noise, not a choice the user needs to make. Groups where every row is fileless still surface (two wishlist records under different ASINs for the same book is a real duplicate). The `DuplicatesReviewModal` grows three tabs (**By ASIN** / **By title/author** / **All**); title/author rows get a **Skip / Discard** action set — Discard routes through `DELETE /library/{id}` per row (not the merge endpoint, since the merge endpoint correctly rejects cross-ASIN attempts) and deletes files + folder when the row has any. Title/author Discards execute before the bulk ASIN merge so a mid-loop failure leaves the merge un-run rather than half-done; per-row failures are collected and surfaced. Confirmation panel describes ASIN-group discards and title/author discards separately. Catches edition variants and wrong-metadata rows the existing same-ASIN dedup tool can't see (e.g. two `Brandon Sanderson / Elantris` rows with ASINs `B00ABC0001` and `B00ABC0002`).
- **Find duplicate audiobooks (Library Maintenance):** New `GET /library/duplicates` returns groups of audiobook rows sharing a normalized (UPPER, trimmed) ASIN, with a server-suggested winner per group. The heuristic prefers rows with files; among those, prefers a single .m4b/.mp3 over a chapter-file import of the same book; then falls back to file count, real book folder shape, folder-name alignment with the book's title/author metadata (the "wrong ASIN got cross-stamped onto a different book's row" tiebreaker), and finally the lowest Id. Each row carries authors, narrators, runtime, tracked-file list (path / size / format / bitrate / duration) and total size so users can verify two rows really are the same book before merging — same ASIN doesn't always mean same content. Each group includes a human-readable recommendation reason ("only row with files", "single-file copy preferred over chapter-file imports", "folder path matches the book's title/author metadata", etc.). New `POST /library/duplicates/merge` resolves groups: optional `WinnerId` (nullable), `LoserIds` (deleted from DB; their `Downloads.AudiobookId` / `History.AudiobookId` / `MoveJobs.AudiobookId` references are reassigned to the winner; their files and folders are also deleted from disk via the same safety-checked `DeleteAudiobookFilesystemAsync` path the single-row delete endpoint uses), and `ClearAsinIds` (the row is kept and its files are left on disk; only its ASIN is nulled — the "wrong ASIN got stamped on this row" case). Filesystem cleanup runs before the DB transaction and surfaces per-row failures as warnings without aborting the rest. Validation requires all losers and clear-asin ids in a pair to share the reference ASIN; on first mismatch the whole request is rejected without changes. Surfaced in Settings → General → Library Maintenance via a "Review duplicates…" button that opens a modal with per-row actions (**Skip / Keep / Discard / Clear ASIN** — all default to Skip), accordion-expandable file details, and an "open in new tab" link to each row's audiobook detail page. Apply now shows a confirmation panel before executing, with a summary of how many rows will be discarded, total file count and bytes that will be deleted from disk, and how many ASINs will be cleared. See kevinheneveld/Listenarr#6.
- **Move-queue summary endpoint surfaces currently-processing jobs, audiobook titles, file counts, and total bytes:** `GET /library/move/summary` now joins MoveJobs → Audiobooks → AudiobookFiles for the rows it returns and the in-scope queued set, returning (a) a `currentlyProcessing` array (one entry per `Status="Processing"` job, ordered most-recently-updated first) so the banner can name the in-flight book instead of just an id, (b) `audiobookTitle`, `fileCount`, and `totalBytes` on every returned job (currently-processing, recent-completed, recent-failed) so size context lands next to the title without a second round-trip, and (c) `queuedFiles` + `queuedBytes` aggregates across every Queued row so the banner can show "queue: N files, M GB remaining" for ETA back-of-envelope vs. observed throughput. Out-of-scope audiobooks (referenced only by long-dead jobs we're not returning) are skipped — single IN-clause query each for files and titles regardless of how many jobs are in scope. Banner rendering updated to surface all of it: a blue "Now moving: <title> (N files, M GB) → <target>" line per processing job, a "Queue remaining: N books · M files · X GB" line, and `(N files · M GB)` size-meta after the title on completed/failed entries. Last-failure text now truncates at 140 chars (was 100) and audiobook title prefixes the error so a runaway path string in the error doesn't make the banner unreadable.
- **Move-queue status banner in the Organize Library modal and Settings → Library Maintenance:** new `MoveQueueStatusBanner.vue` polls `GET /library/move/summary` every 5s while mounted (paused via the Page Visibility API while the tab is hidden) and renders a compact "X of Y done · N processing · N queued · N failed · N other / cancelled" line plus the most recent completed and failed jobs (audiobook id + target / error, with relative timestamps). Mounted in two places: the top of the Organize Library modal body (so the user sees both leftover state from a previous run BEFORE clicking Apply, and live progress AFTER), and the Settings → Library Maintenance section (so the user can see queue status without opening any modal). Renders nothing when there's no move history at all — a brand-new instance doesn't show an empty banner. Color-coded by state: blue when active (queued or processing), green when only completed entries exist, red when there are failures with no completions. No SignalR plumbing — polling is cheap (single table-count query) and avoids the connect / reconnect / fallback complexity of subscribing to MoveJobUpdate from inside a transient modal.
- **One-shot recovery endpoint for audiobooks broken by the pre-fix move queue:** new `POST /library/recover-broken-moves?dryRun=true|false` walks the `History` table for `EventType="Moved"` entries, infers each target path from the entry's `Data` JSON (`{ JobId, Source, Target }`), and — only for audiobooks currently in the broken-state shape (zero tracked `AudiobookFiles` AND `BasePath` is empty or doesn't match the target) — sets `audiobook.BasePath` back to the target and enqueues a scan at the new location with `skipMissingBasePathCleanup=true` (so a recovery scan that races against a manual delete doesn't re-fire the wipe). Verifies the target exists on disk and contains content before touching the DB; an operator-deleted target shows up as `skipped: target_path_missing_on_disk` instead of silently re-stamping `BasePath` to nothing. Defaults to `dryRun=true` so the operator sees what would be recovered before committing; pass `dryRun=false` to execute. Returns counts plus per-row `recoveredDetails` / `skippedDetails` arrays so the failure modes are visible.
- **Move-queue startup rehydration:** `MoveBackgroundService` now calls a new `IMoveQueueService.RehydratePendingAsync(staleThreshold)` at startup that scans the `MoveJobs` table for `Queued` rows (and `Processing` rows older than 5 minutes), flips stale-`Processing` rows back to `Queued`, and re-writes them to the in-memory `Channel<MoveJob>`. Without this, jobs persisted to the DB before a process restart had no consumer and sat forever — and `EnqueueMoveAsync`'s dedup check would silently return their orphan ids on re-queue attempts from the modal. Idempotent against jobs the process already knows about so a rehydration-mid-startup bounce can't double-enqueue. Stale-threshold is conservative: longer than the slowest realistic single-file copy, shorter than the time an operator would wait before noticing the queue stalled.
- **Operator endpoint to cancel stuck pending jobs:** new `POST /library/move/cancel-stale?olderThanMinutes=5` flips every `Queued` or stale `Processing` move job older than the given threshold to `Cancelled`. `MoveBackgroundService` now re-checks each job's DB status before processing — channeled jobs whose DB status flipped to `Cancelled` (or anything other than `Queued`/`Processing`) are skipped as no-ops. Pass `olderThanMinutes=0` to cancel everything pending.
- **Move-queue summary endpoint:** new `GET /library/move/summary` returns counts of persisted `MoveJobs` by status (`queued` / `processing` / `completed` / `failed` / `other`) plus the most-recent N completed and failed jobs (configurable via `recentLimit`, default 25, clamped to [0, 200]). Single table scan; failure messages are included verbatim because they're the most useful post-mortem signal. Enables `curl /library/move/summary` for instant "X of Y moves complete (Z failed)" visibility after queueing a large organize-library run, and is the data source for an in-modal progress banner in a follow-up.
- **Phantom-row cleanup endpoint for hidden duplicates:** `POST /library/cleanup-phantom-rows?dryRun=true|false` targets a population the regular dedup tool deliberately hides: pairs/groups of audiobook rows with the same title + first author where one row has files and the others have zero. These are pre-fix "base path missing" cleanup casualties that nulled the file pointers on one row of an already-duplicate pair, leaving a metadata-only ghost alongside the healthy sibling. The title/author dedup pass filters fileless members of mixed-presence groups out as phantoms (correct for wishlist-vs-real cases) which makes these invisible to the FE. Pure DB cleanup, no filesystem touching — `Downloads.AudiobookId` / `History.AudiobookId` / `MoveJobs.AudiobookId` references on losers get reassigned to the winner, then loser rows are deleted (cascades drop their related rows per the schema). Two-pass union match: a row participates in both passes when it qualifies — Pass 1 keys by normalized ASIN (two rows with the same Audible ASIN are definitively the same book; sanity-checks title+author inside each cluster) and Pass 2 keys by `(title, author, basePath)` with a `loser_asin_mismatch` skip when ASINs disagree. Losers are unioned across passes via a `(loserId → winner)` claim map; a loser claimed by two different winners drops out as `conflicting_winners` for manual review. Clusters with multiple file-owning rows surface as `multiple_winners_in_cluster`. Wraps the DB ops in a single transaction. The dedup tool's phantom filter remains in place — this endpoint exists specifically to surface what that filter hides.
- **Third recovery endpoint for the orphaned-tracking population:** `POST /library/recover-orphaned-tracking?dryRun=true|false` handles the bucket the previous two endpoints don't cover — audiobooks whose `BasePath` points at a specific (non-root) subfolder but whose `AudiobookFile` rows are missing. Same disease as the rootbase population (pre-fix "base path missing" cleanup nulled the file pointers) but the BasePath itself survived, so no inference is needed — the endpoint just verifies the directory exists with audio files (directly or one level deep in a Narrator subdir) and enqueues a re-link scan with `skipMissingBasePathCleanup=true`. Skip reasons: `basepath_missing_on_disk` (folder gone — files truly lost), `basepath_has_no_audio` (folder exists but empty / non-audio content), `scan_enqueue_failed`. Defaults to dry-run.
- **Rootbase recovery falls back to directory search when the canonical pattern doesn't match disk reality:** Live data turned up the population this endpoint exists for has files at *every* depth-1-through-3 layout under their author folder — `/Author/Title`, `/Author/Title/Narrator`, `/Author/Series/Title`, `/Author/Series/Title/Narrator` — not just whatever the configured `FolderNamingPattern` produces. The first cut only matched the canonical pattern (4 of 396). Added a directory-search fallback: when the canonical target doesn't exist on disk, walk under each root folder's `{Author}/` (up to 3 levels) looking for a directory whose normalized name equals the book's normalized title. Performance hardening: the search builds a single in-memory disk index up front (one walk of each root, depth ≤ 3, keyed by `(authorFolderName, normalized title)`) rather than re-walking per book — first cut timed out at the openresty proxy (504 at 90s) on a library of ~400 broken books; the index brings the total runtime to ~25s. Normalization is lowercased alphanumeric only, so "The Crooked Staircase" matches a folder named "The Crooked Staircase" regardless of punctuation/whitespace differences. Match must contain at least one audio file (directly or one level down) so series-level folders aren't false-positives. Skips orphan tmp-staging dirs (`.lna-move-*`, `*.tmp-{guid}`) defensively. Multiple matches → `ambiguous_folder_match` with the candidates surfaced; zero matches → `no_folder_match_under_author` with the searched author + title key for debugging.
- **One-shot recovery endpoint for audiobooks whose BasePath got reset to the library root:** Companion to `recover-broken-moves` for the broader broken-state population — audiobooks whose `AudiobookFile` rows were nulled by the pre-fix "base path missing" cleanup, but whose actual files are still sitting at the canonical target on disk. The first recovery endpoint only catches books with a `Moved` History entry; this one catches the rest by computing the canonical path from each broken row's metadata + the configured `FolderNamingPattern`, verifying the files are physically there on disk, then re-stamping `BasePath` and queuing a re-attach scan. New `POST /library/recover-rootbase-audiobooks?dryRun=true|false` returns per-row `recoveredDetails` (audiobook id, title, previous BasePath, inferred target) and `skippedDetails` with reason (`cannot_compute_target` for rows missing Author/Title, `target_path_missing_on_disk` for books whose files aren't where the pattern would put them, `target_path_empty`, `already_has_files_at_root_basepath`, `recovery_failed`). Defaults to dry-run so the operator can verify the inference matches reality before applying. Recovery scan is enqueued with `skipMissingBasePathCleanup=true` to defend against a race where the operator moves the folder between our existence check and the scan running.

### Fixed
- **MetadataBackfillModal falls back to title/author search when an ASIN returns no usable metadata.** Previously, opening *Compare with online metadata* on an audiobook whose saved ASIN 404s on Audible (regionally restricted, delisted, or returns a 200 envelope containing only the ASIN echo) presented an empty Fresh column with **Apply 0 changes**. The user had to back out, manually clear the ASIN on the audiobook, save, then re-open to coerce the modal into title/author search mode. The auto-load path (start → fetchPreview using the audiobook's existing ASIN) now detects the "no usable metadata" condition — either a thrown error or a successful response with empty title AND empty authors — and transparently runs a candidate search with the audiobook's title + first author. The candidate picker surfaces an informational notice naming the failing ASIN so the user knows why they're seeing a search list instead of a direct edit. User-driven candidate picks (`pickCandidate`) still surface as errors on failure rather than silently fanning out to another search.
- **Organize library preview surfaces apply-time refusals instead of proposing moves that fail:** `GetOrganizePreview` now mirrors two checks `MoveExecutor` performs at execute time, so rows that would fail to move are bucketed as `invalid_target` instead of `will_move`. (1) **Target is an ancestor of source** — when the configured Folder Naming Pattern produces a target one or more directory levels shallower than the source (e.g. source `/audiobooks/Author/Title/Narrator`, target `/audiobooks/Author/Title` after dropping `{Narrator}` from the pattern), the executor refuses with "Target is an ancestor of source; refusing to flatten" because the move semantics get ambiguous. The preview now flags the same case with an actionable reason. (2) **Target directory already exists on disk and contains files** — when the canonical target has foreign content (another audiobook's files, leftover content from a manual move, etc.), the executor refuses with "Target directory already exists and contains files". The preview now stats the target on disk and surfaces this. The canonical-equality case (target == current) is explicitly excluded from check (2) so already-organized rows aren't misclassified as conflicts with their own files. Three new `LibraryController_OrganizeTests` exercise the two new refusal paths and the already-canonical guard against real temp directories; the previously-misleading "20 will move, 0 collisions" preview that produced 18 apply-time failures on the 2026-05-27 organize run would now show those rows as invalid_target with reasons the operator can act on.
- **Move-queue status banner hides when the queue is idle:** the banner's visibility predicate was `summary.total > 0`, which included historical Completed / Failed / Cancelled counts from prior sessions — so the banner persisted indefinitely showing stale "665 of 767 done · 102 failed" tombstones long after work was over. Now only renders when `processing > 0 || queued > 0`, with a 10-minute grace window after the most recent failure so a fresh failure isn't missed if the user wasn't watching when the toast fired. `loadError` still forces the banner up so backend issues stay visible.
- **Rename / `FileMover.MoveDirectoryAsync` no longer wipes the library root on bad inputs:** Audit prompted by the organize-library near-miss turned up the same catastrophic-delete shape in the rename flow — `FileMover.MoveDirectoryAsync`'s fallback path is `CopyDirRecursive(src, dst)` + `Directory.Delete(src, recursive: true)`, and the method had zero input validation. A rename triggered on one of the 263+ books whose `BasePath` got reset to `/audiobooks` would call `MoveDirectoryAsync("/audiobooks", "/audiobooks/Author/Title")`, which (a) recursively copies the library root into a subfolder of itself (the same recursion pattern fixed in the move queue) and (b) deletes the library root afterward, wiping every other audiobook. Three layers of defense added: (a) `FileMover.MoveDirectoryAsync` now refuses *before* any FS work when source is empty, destination is empty, source equals destination (returns success no-op), destination is inside source, source is inside destination (flattening), or source's parent resolves to a filesystem root (Linux `/`, Windows drive root). (b) `RenameService.ExecuteDirectoryMoveAsync` adds a strict-equality check against the configured library roots — `IsPathWithinAllowedRoots` returns true for "root is within itself," so a dedicated `IsPathAConfiguredRoot` is needed to catch and refuse the library-root case with an operator-actionable message. (c) Seven new `FileMoverSafetyTests` exercise each refusal path with real temp directories, including a Linux-only test that creates `/lna-filemover-safety-{guid}` as the source to verify the filesystem-root guard fires before the destructive fallback runs.
- **Organize-library refuses audiobooks whose BasePath is the library root itself:** A latent companion to the recursion bug — when an audiobook's `BasePath` was incorrectly stamped at a configured root folder (e.g. `/audiobooks`) instead of at its own subfolder, the organize-library flow would compute a target *inside* source (so `source ⊂ target` was true), `MoveExecutor` would stage outside source at `Path.GetDirectoryName("/audiobooks") = "/"`, and the staging dir creation would fail with permission-denied. **The much worse failure mode that was narrowly avoided**: had staging succeeded, the post-copy `Directory.Delete(source, true)` would have deleted the entire library root and every other audiobook on it. The recursion-fix's "stage outside source" path inadvertently saved us by hitting the permission wall before the delete step. Three layers of defense added: (a) `GetOrganizePreview` now routes any row whose normalized `BasePath` equals a configured `RootFolder.Path` into the `invalid_target` bucket with reason "Source path is the library root folder. Re-scan the library so this audiobook's BasePath points at its actual subfolder before organizing." — those rows never reach the move queue. (b) `ApplyOrganize` repeats the same check before enqueueing, in case the FE snapshot is stale or the operator hits the endpoint directly. (c) `MoveExecutor.ExecuteMoveAsync` refuses with a clear "source has no usable parent directory (looks like a library root)" error when `Path.GetDirectoryName(source)` resolves to the filesystem root — last line of defense if both gates are bypassed. New `MoveExecutorTests.SourceAtFilesystemRoot_Refused` exercises the executor refusal against a real source whose parent is "/" (Linux only; auto-skips on Windows / when the sandbox can't mkdir at root). The 83 failures from the 2026-05-26 organize run that surfaced this surfaced cleanly via the existing permission-denied path; data was not lost.
- **Move queue no longer recurses catastrophically when the target is inside the source:** `MoveBackgroundService` placed its staging directory under `Path.GetDirectoryName(target)` and enumerated the source recursively. When the configured `FolderNamingPattern` produced a target that lived under the current source — most commonly when adding a `{Narrator}` level under an existing `{Author}/{Title}` (e.g. source `/audiobooks/Sanderson/Mistborn`, target `/audiobooks/Sanderson/Mistborn/Michael Kramer`) — `targetParent == source`, so the staging dir landed inside source's enumeration scope. The recursive copy then discovered its own output and copied tmp into tmp into tmp until the OS gave up on path length, surfacing as a deeply-nested "tmp-{guid}/tmp-{guid}/…" error message that bloated `History` rows. Fix: extract the copy-then-rename logic into `MoveExecutor.ExecuteMoveAsync` (new file in `listenarr.infrastructure/FileSystem/`), which now (a) detects `target ⊂ source` via prefix comparison on normalized full paths with trailing separator, (b) places the staging dir as a sibling of source (one level outside the recursion scope) in that case, instead of under `targetParent`, (c) renames the new tmp pattern to `.lna-move-{jobId:N}` — leading dot keeps it out of normal listings, the `lna-move-` segment is greppable from the host, (d) refuses the inverse `source ⊂ target` (flattening) case explicitly rather than producing a half-renamed half-empty mess, and (e) defensively filters its own staging path out of the enumeration during the copy loop. Surfaced error messages are capped at 500 chars at the source so a runaway path can't make a single failure unreadable in History / toasts. Companion operator endpoint `POST /library/move/cleanup-orphan-tmp?dryRun=true` sweeps root folders for both the new `.lna-move-{guid}` staging dirs and the legacy `name.tmp-{guid}` pattern left behind by interrupted moves under the old code path; dry-run by default, returns per-candidate size info so an operator can size the cleanup before committing. Covered by 10 integration tests against real temp directories (`MoveExecutorTests`) that exercise the source-contains-target case, sibling moves, no-ops, non-empty-target refusal, the target-contains-source refusal, content preservation, the truncation cap, and the orphan-enumeration patterns.
- **Library moves no longer orphan their files from the audiobook record:** `MoveBackgroundService` copied files from source to target, deleted the source directory, then enqueued a post-move scan via `EnqueueScanAsync(audiobook, null)` — but never updated `audiobook.BasePath` to point at the new location. The scan therefore ran against the audiobook's **stale BasePath** (the now-empty source path), reported every tracked file as missing, fired `File Removed (base path missing)` via the "base path missing" cleanup, and nulled `BasePath`. Net effect: every successful move silently broke the library record for that audiobook — files were on disk at the canonical target, but the DB row had `BasePath=null` and an empty `AudiobookFiles` list. The bug also explained why the organize-library preview never transitioned successfully-moved rows from `will_move` to `already_canonical`: the row's `BasePath` was still the old path (or `null` after the cleanup), so the bucket comparison always disagreed. Fix: set `audiobook.BasePath = FileUtils.NormalizeStoredPath(target)` and persist BEFORE enqueueing the scan. The scan now sees the new location, re-attaches the (just-moved) files, and the next preview correctly classifies the row. DB-update failure here is non-fatal — files are already at the target on disk; the operator gets a loud error log pointing them at the affected audiobook id so a re-scan can recover.
- **Organize-library preview drops fileless rows whose `BasePath` is non-empty too:** the earlier filter skipped a row only when BOTH `BasePath` was empty AND there were no tracked files. Live data turned up a third pattern — monitored-but-not-downloaded records whose `BasePath` had been stamped at the library root (e.g. `/audiobooks`) by an earlier ingestion path. Those rows survived the narrow filter, appeared in `will_move`, got default-selected on Apply, and each failed `MoveBackgroundService`'s "source path invalid or does not exist" check one by one — generating per-job DB writes and SignalR pings without doing real work. The filter now drops every row with zero tracked files regardless of `BasePath` value; if there's nothing on disk to move, the row is invisible to organize-preview. Reduced one live library's `will_move` count from 1419 to 636 (-783 doomed-to-fail jobs) in a single query.
...
- **Organize-library preview no longer surfaces wishlisted rows with nothing on disk:** `LibraryController.GetOrganizePreview` routed every row with a non-empty computed target and a different current path into the `will_move` bucket, including audiobooks that were monitored but never downloaded (no tracked `AudiobookFiles` and an empty `BasePath`). With "select all" being the modal's default state, applying the preview would queue per-book moves for those rows and every one would fail with "Source path does not exist." The preview now skips audiobook rows whose `BasePath` is empty AND that have zero tracked files entirely — they contribute to no bucket, no count, and are invisible to the apply step. Audiobook rows with a `BasePath` but missing files (or files but no `BasePath`) still surface for diagnosis as before.
...
- **Backfill candidate search collapses to the obvious match when one exists:** After the narrow author-page search + the title-only supplement merge into a single candidate set, the AUTHOR_TITLE branch now scans the merged list for "exact matches" — candidates whose normalized title equals the user's typed title AND whose author overlaps the typed author — and if any exist, returns only those. Common case it fixes: searching "Foundation" by Asimov used to return every Foundation-named title in the catalog (Foundation, Foundation and Empire, Second Foundation, Foundation's Edge, Foundation and Earth, Prelude to Foundation); now it returns only the one whose title is literally "Foundation". Multiple legitimate exact matches (e.g. same book, different narrators / abridgements) are all kept so the user can still pick the narrator they own. When no candidate exactly matches, the full merged list is returned unchanged so the user can disambiguate. The normalization keeps only letters and digits and is case-insensitive, so common punctuation differences ("&" vs "and", colons, hyphens) don't block equality, while distinct titles like "Foundation" vs "Foundation and Empire" still stay distinct. The author check is a substring contains on the normalized author so "Stephen King" matches "Stephen King" but also accepts the user typing just "King" or "Stephen".
...
- **"Preview my file" inside the metadata-backfill modal now stacks above the parent overlay:** The shared `Modal` component's overlay defaults to `z-index: 3000` and `MetadataBackfillModal`'s overlay uses `3100`, so opening `FilePreviewModal` from inside the backfill modal rendered the preview *behind* the parent overlay — the screen darkened but no audio controls appeared. `Modal` gains an optional `overlayZIndex` prop, `FilePreviewModal` forwards it, and the backfill modal's usage passes `3200`. Default behaviour for every other Modal caller is unchanged.
...
- **Backfill modal no longer flags `publishedDate` as changed when only the format differs:** Audible returns publish dates as ISO timestamps (`2019-06-04T07:00:00Z`); the library stores bare dates (`2019-06-04`). The comparison view's string-trim equality marked these as different and left "publish date" available to overwrite with what amounted to a no-op. The `sameValue` comparator now date-normalizes both sides before comparing for the `publishedDate` field — extracting the leading `YYYY-MM-DD` portion. Same-date values now correctly appear unchanged and are not selectable.
- **Music-album downloads are rejected at the import stage:** A new pre-ingest verification step runs in `DownloadImportService` after files land on disk but before they are committed to the library. It extracts ffprobe metadata for every audio file in the release up front (and reuses that cache in the per-file loop so it costs no extra ffprobe calls), then rejects the whole batch when it looks like a music album — ≥10 audio files with a median track length under 5 minutes. Catches releases that slipped past the indexer-category and title-relevance filters. Conservative by design: fails open when the duration signal is missing (no ffprobe output) and on batches below the file-count threshold.
- **Backfill candidate search no longer drops matches over punctuation differences:** The Advanced-mode `POST /search` path narrowed Audible author results to a specific title with a naive case-insensitive `IndexOf`. That dropped legitimate hits whenever the user-entered title differed from Audible's by punctuation alone — e.g. searching "1634 - The Baltic War" (hyphen) against author "David Weber" returned zero matches even though Audible has "1634: The Baltic War" (colon, ASIN B00F8LD0QM). A new `TitleMatcher` normalizes both sides (lowercase, strip punctuation including `-` / `–` / `—` / `:` / `;` / `,` / `.` / `()` / `[]` / smart quotes, collapse whitespace) before substring-matching, with a tight token-overlap fallback (≥2 significant query tokens, all must appear) to catch reordered/decorated titles without admitting unrelated books. Subtitle is checked the same way. Single-token queries skip the overlap fallback so a bare "1634" does not match every book that contains that word.
- **Backfill modal: candidate search returns real matches, and title/author are editable inline:** Two bugs ran together. (1) `searchAudibleByTitleAndAuthor` typed the `POST /search` response as a `{ totalResults, results }` envelope, but the backend actually returns a flat array of Audible-shaped results (see `Ok(flatMapped)` in `SearchController.cs`). The mismatch silently dropped every hit — the modal would say "no matches" even when the unified Audible+Audnexus+OpenLibrary search had found one (e.g. "A Darkness Upon the Ice" by William R. Forstchen, ASIN B009KS9JKA). The wrapper now normalises both shapes. (2) The empty-results screen advised editing the title or author, but neither was editable from inside the modal, making the "Search again" button pointless. The candidate-picker now exposes inline title/author inputs that drive the retry, the help copy reflects that, and the error message no longer claims the search was Audible-only.
...
- **Automatic search no longer matches results whose title is irrelevant to the audiobook:** A new context-aware `RelevanceFilter` participates in the search-result filter pipeline. When automatic search runs, the pipeline is invoked with the target audiobook so the filter can compare the result title against the audiobook's title + authors via stop-word-filtered token overlap. Below 30% overlap → rejected. Defends against indexers returning matter that shares one or two tokens with the query — e.g., a Patterson Hood concert recording being matched to a James Patterson audiobook on the single shared surname. Manual search keeps the old behavior (no context passed → filter fails open) so users browsing results still see everything. Automatic search also now restricts Newznab queries to the "Books > Audiobook" category (`3030`) so music-only indexers configured in Prowlarr without a category restriction don't return concert/album torrents for audiobook queries. The stop-word-filtered tokenizer is lifted into a shared `SignificantTokens` helper so the upcoming backfill TitleMatcher can reuse it.

### Added
<!-- Pre-today UI/feature work for this release — kept below the infrastructure block that opened the version, in roughly the chronological order it landed. -->
...
- **Backfill modal accepts a pasted Audible URL or ASIN as an escape hatch when search comes up short:** Audible's search endpoints don't always surface every record they actually serve — recent releases or alternate editions sometimes return full metadata via the per-ASIN endpoint while being entirely absent from keyword / title / author-page search responses (e.g. "Robots and Empire" / B0CSV7NJMB is reachable at `/metadata/B0CSV7NJMB` but never appears in any of our search-side calls, so the candidate picker can't reach it). The pick-candidate phase now exposes a second input under the title/author search form: paste a bare 10-character ASIN or any audible.com URL containing one and click Load — the modal extracts the ASIN via regex (`/\b([0-9A-Z]{10})\b/i` so it survives `srsltid`, `ref`, trailing slashes, and store-domain variations) and routes through the existing `pickCandidate(asin)` path so the downstream comparison view loads identically to a click-from-the-list flow. The field is reset on every modal open so a stale value from a previous book never re-applies.
- **Audition the owned file from the metadata-backfill modal:** A "Preview my file" button appears next to the book title in the metadata-backfill comparison view. Clicking it opens the existing in-browser audio preview (`<audio controls>` over the streaming endpoint) loaded with the audiobook's first audio file, so the narrator's voice can be identified before committing to one of several Audible candidates that may differ only by narrator or abridgement. For multi-file books the button auditions the first file (usually the intro / chapter 1); the tooltip flags the file count so the user knows what they're hearing. Reuses `FilePreviewModal` with no new backend endpoints.
- **Opt-in full metadata rescan for existing audiobooks:** The Library Import page grows a "Full metadata rescan" checkbox alongside the Scan button; ticking it also fires `POST /api/v1/library/backfill-metadata`, which enqueues a force-refresh scan for every audiobook in the library. The Edit Audiobook modal grows a "Fill missing from file" button next to "Fill missing from online…" that does the same for a single book via `POST /api/v1/library/{id}/scan` with `forceMetadataRefresh: true`. When the flag is set and the file is already tracked, `AudiobookFileService.EnsureAudiobookFileAsync` re-extracts metadata via ffprobe and re-runs the Pass A promotion against the parent audiobook — filling blank fields (title/subtitle/series/series-pos/publisher/language/desc/narrator/authors/ASIN/ISBN) and importing embedded cover art into local library storage. Existing non-blank values are never overwritten. Library-wide backfill additionally sets `ScanJob.SkipMissingBasePathCleanup` so a single stale BasePath cannot cascade into wholesale `AudiobookFile` deletion. Promotion now returns whether anything mutated so multi-file books skip no-op `UpdateAsync` calls after the first file has filled every blank.
- **In-browser audio preview for owned files:** Each row in an audiobook's file list grows a play button that opens a small modal with a native `<audio controls>` element streaming the file directly. Designed for the "I have three editions of this book by different narrators and no way to tell which is which" use case — scrub a few seconds in, listen for the voice, close the modal. Backed by a new `GET /api/v1/library/{id}/files/{fileId}/stream` endpoint that serves the file with HTTP Range support so scrubbing a multi-hundred-MB m4b doesn't download the whole file. Authentication piggybacks on `SessionAuthenticationMiddleware`'s session-cookie fallback (the same path that already lets the browser fetch cached images), so no new tokens or header workarounds were needed. The endpoint validates that (a) the requested file belongs to the URL's audiobook, (b) the resolved absolute path sits under one of the configured root folders, (c) the file isn't a reparse-point symlink, and (d) the extension is one of `mp3 / m4a / m4b / mp4 / aac / ogg / oga / opus / flac / wav / webm` — DRM-protected `.aax` is intentionally not in the list since browsers can't decode it. Failed playback (wrong codec for this browser, file moved on disk) shows a single inline error in the modal rather than a console-only failure.
- **Backlog sweep for external cover art:** New admin action `POST /api/v1/library/cache-external-covers` walks every audiobook and downloads any cover still pointing at an external `http(s)://` URL into local library storage, then rewrites the stored URL to the local cache path. Reuses the same helper (`LibraryImageStorageHelper`, extracted from `LibraryController`) and cache-key derivation as the on-save hook so cached files land under the audiobook's ASIN — or a deterministic hash of the first ISBN, or title + first author when neither is set. Idempotent: records already pointing at `/cache/`, `/config/cache/`, or `/api/v1/images/...` are skipped, including fully-qualified URLs that happen to resolve back to those routes. Per-record failures (download 404, IO error) are logged and counted but do not abort the sweep; the external URL is left in place so the user still sees a cover. Amazon CDN requests are throttled with a 250ms pause between successful downloads. Response shape: `{ message, totalScanned, alreadyLocal, queued, succeeded, failed, durationMs }`. Surfaced in the UI as a "Run sweep" button in a new Library Maintenance section on the General settings tab — no new navigation. Closes the historical-backlog gap left open by the on-save caching hook.
- **Online metadata-backfill with preview:** The edit-audiobook modal grows a "Fill missing from online…" action. It opens a comparison modal that fetches the book's Audible metadata and shows it side-by-side with the current library values field-by-field. Each field has a checkbox; the default is OFF when the book already has a value (overwriting existing data is explicit) and ON when the field is empty. Cover-art rows show both images. Apply submits only the selected fields via the existing library-update endpoint. When the book has no ASIN, the modal first runs an Audible search by title + first author and lets the user pick the matching candidate; the review view also has a "Not this book? Search by title" escape hatch when the wrong ASIN is on file.
- **Library metrics dashboard:** A new `/dashboard` page (and nav entry) presents aggregate metrics for the whole library, served by a new `GET /api/v1/dashboard/stats` endpoint. The dashboard is book-centric: it leads with books actually owned (file present on disk) vs. tracked-but-missing, and the author/narrator sections show per-contributor have-vs-missing as stacked bars so a large "tracked" number doesn't hide how few are owned. Other sections cover per-field metadata completeness with an overall score, series completeness (owned books vs. the cached Audible catalog), file quality (codec and bitrate), genre/duration/language distributions, and a books-added activity time-series with an adjustable Day/Week/Month granularity. Audible labels many standalone books as 1-member "series" — those are folded into the standalone count rather than counted as real series. Charts use ApexCharts; the route and chart library are lazy-loaded so they add nothing to the initial bundle.
- **Automatic series catalog caching:** A background service now caches the Audible catalog for any series present in the library, not just series the user has explicitly monitored — so the dashboard's series-completeness numbers resolve over time instead of staying "unknown". It only considers series with 2+ books in the library, skips already-cached series, and rate-limits Audible fetches. Caching is not monitoring (it never triggers searches). Opt out with `LISTENARR_AUTO_CACHE_SERIES_CATALOGS=false`.
- **Add-series bulk action on the Add New page:** Search results that belong to a series now show an "Add series" button next to "Add to Library". It opens a modal that fetches the Audible series catalog, lists every book in the series with already-owned ones marked, and lets you bulk-add the rest with shared options (root folder, quality profile, monitored, auto-search). There's also an opt-in to monitor the series itself so future releases are picked up by automatic search. Books are added one at a time via the existing add endpoint, so per-book failures don't stop the rest. No new backend code.
- **Dashboard drill-down into the audiobooks library:** Dashboard counts and chart segments now deep-link into the audiobooks page with a preset filter applied, shown as a clearable chip at the top of the list. The metadata-completeness bars and the "Books missing" overview tile link to `?missing=<field>`. Author, narrator, and genre stacked-bar segments link to `?author=<name>` / `?narrator=<name>` / `?genre=<name>`, and a click on the Missing segment additionally adds `&missing=files` so the destination list is scoped to just that contributor's missing books (the "Patterson missing 47 of 400" use case). The languages donut slices route to `?language=<name>`. Filters compose, and the chip summarises the active set. The audiobooks page already loads the library client-side, so the drill-down adds no new backend endpoint. Stacked have/missing bars were also added to the genres and duration distribution sections (matching the authors/narrators pattern), and the activity time-series now shows two overlaid lines — *Requested* (books added to the library) and *Imported* (files actually arrived) — so the queue-to-arrival lag is visible.
...
- **Per-file rename on the audiobook detail page:** Pencil icon on each row of the Files tab opens a modal for renaming a single tracked file. The modal calls the existing `POST /api/v1/library/{id}/rename/preview` endpoint to obtain the authoritative current path (mirroring how the bulk organize flow handles relative-stored paths), then lets the user edit only the filename portion — the extension is preserved and path separators are rejected. Submitting issues a single-file `FileRenameOperation` through `POST /api/v1/library/{id}/rename`; other files in the audiobook are unaffected. No backend changes — `RenameService` already accepts arbitrary `NewPath` values in per-file operations.
...
- **Cache external cover art locally on save (opt-in):** `PUT /library/{id}` accepts a new `cacheImageLocally` query parameter. When `true` and the post-merge `ImageUrl` is an external `http(s)://` URL, the controller downloads the cover into local library storage and rewrites the stored URL to the local cache path (mirroring the existing add-path's `MoveMetadataImageToLibraryStorageAsync` hook). Works whether the URL is new in this request or was already stored — so users can convert an existing record's external cover to local just by re-saving with the checkbox ticked. No-op when the URL is already local or the caller omitted the field. The metadata-backfill modal always sends the flag — the whole point of that flow is to import fresh metadata into the library. The edit-audiobook modal exposes a new "Download and cache cover art locally on save" checkbox under the Cover Image URL field, shown only when the URL is external and defaulted ON so the common case (paste-and-save or convert-on-edit) just works. Once the URL flips local, the checkbox auto-hides and the flag stops being sent, so subsequent unrelated edits don't re-download the cover. Cache failures fall back to storing the external URL, matching the add-path. The historical-backlog sweep (rewriting existing rows in bulk) is deliberately left as a separate concern.
