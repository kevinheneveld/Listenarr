...
namespace Listenarr.Tests.Features.Infrastructure.Library.Moving
{
    ...
    public partial class MoveJobProcessorTests : BaseTests
    {
        ...
        public async Task ProcessJobAsync_StubTarget_RefusedWithoutFlag()
        {
            ...
            var updatedJob = await queue.GetJobAsync(job.Id);
            // #717's durable-move model parks an occupied target as NeedsAttention
            // (operator review) instead of terminal Failed; the protective intent —
            // nothing at either side is destroyed — is unchanged.
            Assert.Equal(MoveJobStatus.NeedsAttention, updatedJob!.Status);
        ...
        public async Task ProcessJobAsync_AudioInTarget_RefusedDespiteFlag()
        {
            ...
            var updatedJob = await queue.GetJobAsync(job.Id);
            // Occupied-audio target parks as NeedsAttention under #717; the audio
            // in the target must survive regardless of the replace flag.
            Assert.Equal(MoveJobStatus.NeedsAttention, updatedJob!.Status);
            Assert.True(File.Exists(Path.Join(src, "book.m4b")));
