...
public sealed class AudiobookScanServiceTests : BaseTests
{
    ...
    // The mid-extraction swap protection this asserts comes from reading
    // metadata through the pinned handle's /proc fd path — a Linux-only
    // mechanism; Windows and macOS extract via the canonical path by design
    // (see PinnedAudiobookFileRegistrationLease).
    [LinuxFact]
