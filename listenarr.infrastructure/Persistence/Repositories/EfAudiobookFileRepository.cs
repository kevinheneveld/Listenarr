...
namespace Listenarr.Infrastructure.Persistence.Repositories
{
    public partial class EfAudiobookFileRepository : IAudiobookFileRepository
    {
        ...
        public async Task ReassignAsync(int fileId, int newAudiobookId, string? newPath, CancellationToken ct = default)
        {
            ...
            var stub = new AudiobookFile { Id = fileId, AudiobookId = newAudiobookId };
            var entry = _db.Entry(stub);
            entry.Property(f => f.AudiobookId).IsModified = true;
            if (newPath != null)
            {
                // Path's setter is domain-internal; write through the EF entry,
                // which assigns via the backing field.
                entry.Property(f => f.Path).CurrentValue = newPath;
        ...
        public async Task<Dictionary<int, long>> GetSizeSumsByAudiobookIdAsync(CancellationToken ct = default)
        {
            return await _db.AudiobookFiles
                .AsNoTracking()
                .GroupBy(f => f.AudiobookId)
                .Select(g => new { AudiobookId = g.Key, Size = g.Sum(f => f.Size ?? 0) })
                .ToDictionaryAsync(r => r.AudiobookId, r => r.Size, ct);
        }
