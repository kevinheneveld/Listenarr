...
internal sealed partial class PinnedDirectoryCreation
{
    ...
    private static bool MacHandlesIdentifySameDirectory(
    ...
    // openat(2) is variadic: its mode argument travels on the stack under the
    // Apple arm64 varargs ABI while a fixed-signature DllImport passes it in a
    // register, so O_CREAT opens create files with a garbage mode on macOS
    // (the same hazard this file documents for fcntl). The creating fd is not
    // restricted by the file mode, so a non-variadic fchmod immediately after
    // creation deterministically repairs the permissions.
    [DllImport("libc", EntryPoint = "fchmod", SetLastError = true)]
    private static extern int FchmodUnix(int fd, uint mode);

    private static void RepairCreatedFileModeMacOs(int fd, uint mode)
    {
        ...
        if (FchmodUnix(fd, mode) != 0)
        {
            throw new Win32Exception(
                Marshal.GetLastWin32Error(),
                "Could not normalize the mode of a newly created pinned file.");
        }
    }
