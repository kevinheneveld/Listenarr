...
public sealed class FileSystemSemanticsResolver : IFileSystemSemanticsResolver
{
    ...
    private const int MacPathconfCaseSensitive = 11; // _PC_CASE_SENSITIVE (Darwin)

    private static FileSystemSemanticsResolution ResolveMacOs(
        string boundary,
        FileSystemPathSyntax syntax)
    {
        // pathconf(_PC_CASE_SENSITIVE) is macOS's read-only per-volume answer:
        // 1 = case-sensitive (APFS/HFS+ sensitive variants), 0 = insensitive
        // (the default), -1 with errno = error, -1 without errno = undefined
        // (treat as the platform default: insensitive).
        System.Runtime.InteropServices.Marshal.SetLastSystemError(0);
        var value = PathconfMacOs(boundary, MacPathconfCaseSensitive);
        if (value < 0)
        {
            var error = System.Runtime.InteropServices.Marshal.GetLastSystemError();
            if (error != 0)
            {
                return Unavailable(
                    syntax,
            ...
            return Valid(syntax, boundary, FileSystemCaseSensitivity.Insensitive);
        }
    ...
    private static extern long PathconfMacOs(string path, int name);
