        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var filePath in attributedFiles)
        {
            ...
            try
            {
                ...
                else if (command.ForceMetadataRefresh)
                {
                    // Rescan-metadata behavior: the file is already tracked; re-extract
                    // its metadata and backfill blank library-level fields.
                    await fileService.RefreshTrackedFileMetadataAsync(
                        audiobook,
                        filePath,
                        cancellationToken);
                }
