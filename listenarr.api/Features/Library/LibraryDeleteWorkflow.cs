...
namespace Listenarr.Api.Features.Library
{
    public sealed partial class LibraryDeleteWorkflow
    {
        ...
        public Task<IActionResult> DeleteAsync(
            ...
            bool deleteFiles,
            bool deleteFolder,
            bool excludeFromAuthorMonitoring = false,
        ...
        private async Task<IActionResult> DeleteCoreAsync(
            ...
            bool deleteFiles,
            bool deleteFolder,
            bool excludeFromAuthorMonitoring,
            CancellationToken cancellationToken)
        {
            // Record an exclusion BEFORE deleting so a monitored author's or
            // series' sweep doesn't just re-discover this book as "missing"
            // and add it straight back.
            if (excludeFromAuthorMonitoring)
            {
                var toExclude = await _audiobookRepository.GetByIdAsync(id);
                if (toExclude != null)
                {
                    await _authorMonitoringService.AddExclusionForAudiobookAsync(toExclude, cancellationToken);
                }
            }
